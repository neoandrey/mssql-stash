...
IF  (OBJECT_ID('tempdb.dbo.#settlement_summary_session') IS NOT NULL)
BEGIN
     DROP TABLE #settlement_summary_session;
END

IF  (OBJECT_ID('tempdb.dbo.#settlement_summary_breakdown') IS NOT NULL)
BEGIN
     DROP TABLE #settlement_summary_breakdown;
END

IF  (OBJECT_ID('tempdb.dbo.#report_result') IS NOT NULL)
BEGIN
...
CREATE TABLE #settlement_summary_session (
	[Business_Date] [varchar] (100)  NOT NULL , 
)
...
--INSERT 
      INSERT INTO #settlement_summary_session          --               INTO settlement_summary_session
       SELECT 
       
         distinct (substring (CAST (y.trans_date AS VARCHAR(8000)), 1, 11)) + '_Reward'  AS Business_Date
...
GROUP BY pt.extended_tran_type,
substring (CAST (pt.recon_business_date AS VARCHAR(8000)), 1, 11)	
-- y.trans_date
...
CREATE TABLE [#settlement_summary_breakdown] (
	[id] [bigint] IDENTITY (1, 1) NOT NULL ,
	[bank_code] [varchar] (32) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	[trxn_category] [varchar] (64) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	[Debit_Account_Type] [varchar] (100) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	[Credit_Account_Type] [varchar] (100) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	...
	[Late_reversal] [char] (1) COLLATE SQL_Latin1_General_CP1_CI_AS NULL 
) 
...
SELECT		distinct
	 ...
	 CASE when (substring(y.extended_trans_type,1,1) = '8') then 'SAVERSCARD REWARD SCHEME' 
          ...
          isnull('FEE POOL(Debit_Nr)','na') AS Debit_account_type,
                          
      isnull('ISW_Reward_Fee_Receivable (Credit_Nr)','na') AS Credit_account_type,  
                          
        0 AS Amt,

	isnull(CASE WHEN (substring(y.extended_trans_type,1,1) = '8') 
              AND (dbo.fn_rpt_Above_limit (abs(pt.settle_amount_impact/100),c.amount_cap ) = 1) 
                  THEN (sum((r.reward_discount*(-1*(settle_amount_impact)))-(c.merchant_disc*(-1*(settle_amount_impact)))))*0.5
