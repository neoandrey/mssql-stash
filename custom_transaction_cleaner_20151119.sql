...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 11/19/2015 09:00:10 ******/
SET ANSI_NULLS ON
GO
	   ...
	   --SET @datetime_start=DATEADD(MINUTE, @period_interval, @datetime_temp);  
		...
		WHILE (DATEDIFF(MINUTE,@datetime_start, @endDate)>0)   
		 BEGIN  
		  ...
		  DELETE_POST_TRAN:  
		  DELETE FROM post_tran  WHERE post_tran_id <=  @last_post_tran_id   AND   datetime_req  <= @datetime_stop;  
		  IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN  
		  SET ROWCOUNT 0  
	  
		  SET ROWCOUNT @batch_size  
		  DELETE_POST_TRAN_CUST:  
		  DELETE post_tran_cust FROM post_tran WITH(NOLOCK, INDEX(ix_post_tran_7)) INNER JOIN post_tran_cust ON post_tran.post_tran_cust_id = post_tran_cust.post_tran_cust_id WHERE  post_tran_id <=  @last_post_tran_id  AND datetime_req < @datetime_stop;  
		 ...
		 END  
		  ...
		  DELETE FROM post_tran   WHERE  post_tran_id <=  @last_post_tran_id AND datetime_req < CONVERT(DATETIME, @datetime_stop);  
		  IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN_ALL  
		  SET ROWCOUNT 0  
	  
		  SET ROWCOUNT @batch_size  
		  DELETE_POST_TRAN_CUST_ALL:  
		  DELETE post_tran_cust FROM post_tran WITH(NOLOCK) INNER JOIN post_tran_cust ON post_tran.post_tran_cust_id = post_tran_cust.post_tran_cust_id WHERE  post_tran_id <=  @last_post_tran_id AND datetime_req < CONVERT(DATETIME, @datetime_stop) ;  
