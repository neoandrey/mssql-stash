...
END
INTO post_settle_tran_details_20160914
...
FROM   temp_post_tran_data PT (NOLOCK )    
    ...
    (
       ...
       or (
       (PT.PT_settle_amount_impact<> 0 
       and PT.PT_message_type = '0420'    
     ...
     OR (
       ...
       or (PT.PT_settle_amount_rsp<> 0 and PT.PT_tran_type = '40'  and PT.PT_message_type   in ('0200','0220') and (SUBSTRING(pt.PTC_Terminal_id,1,1) IN ('0','1') ))
       or (PT.PT_message_type = '0420' and PT.PT_tran_reversed <> 2 and PT.PT_tran_type ='40' and (SUBSTRING(pt.PTC_Terminal_id,1,1)IN ( '0','1' ))))
     

      AND not (  PT.PT_tran_type = '00'  and  abs(PT.PT_settle_amount_impact/100)< 200 and pt.PTC_merchant_type in ('4004','4722') and pt.PTC_source_node_name not in ('VTUsrc','CCLOADsrc') 
...
left JOIN tbl_merchant_category m (NOLOCK)
				...
				ON (PT.PTC_terminal_id= y.terminal_id 
                                    ...
                                    AND substring (CAST (PT.PT_datetime_req AS VARCHAR(8000)), 1, 10)
                                     convert(VARCHAR(10),PT. PT_datetime_req,112 ) 
