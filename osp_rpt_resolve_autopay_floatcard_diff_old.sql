...
GO
/****** Object:  StoredProcedure [dbo].[osp_rpt_resolve_autopay_floatcard_diff]    Script Date: 01/07/2015 08:32:39 ******/
SET ANSI_NULLS ON
GO
...
SELECT  batch_identifier, retrieval_reference_nr, settle_amount_impact,settle_tran_fee,transaction_count, total_settle_tran_fee,total_settlement_impact, (total_settle_tran_fee+total_settlement_impact) outstanding_amount FROM #transaction_batch_data WHERE  CHARINDEX('deposit',tran_type_description) <= 0 AND (total_settlement_impact+total_settle_tran_fee) <> 0 AND retrieval_reference_nr NOT IN (SELECT retrieval_reference_nr FROM #temp_switch_data)  ORDER BY batch_identifier
