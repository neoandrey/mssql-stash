...
GO
/****** Object:  StoredProcedure [dbo].[osp_rpt_b04_web_pos_acquirer_nibss]    Script Date: 07/10/2016 16:52:25 ******/
SET ANSI_NULLS ON
GO
...
GO
    ...
    CREATE                   PROCEDURE [dbo].[osp_rpt_b04_web_pos_acquirer_nibss]
...
tran_cash_rsp  FLOAT,
	...
	BEGIN	   
	   	INSERT INTO @report_result2 (Warning) VALUES ('Please supply the Web channel source node name.')
...
IF (@StartDate IS NULL OR @StartDate ='') 
	...
	BEGIN
	   	INSERT INTO @report_result2 (Warning) VALUES ('The End Date must be AFTER the Starting Date.')
	   	SELECT * FROM @report_result2
		RETURN 1
	END
...
INSERT
	...
	FROM
				...
				ON  t.post_tran_cust_id = t.post_tran_cust_id
			 ...
			 JOIN (
				...
				ON
					rec.recon_business_date = y.trans_date
					left JOIN
...
dbo.formatAmount(t.tran_cash_req,t.tran_currency_code) AS tran_cash_req, 
    ...
    LEFT( t.source_node_name,2)  <> 'SB'
         ...
         --and t.post_tran_cust_id > '946318193'      
     OPTION (recompile, MAXDOP 8)
...
dbo.formatAmount(q.tran_cash_req,q.tran_currency_code) AS tran_cash_req, 
	...
	FROM
				asp_visa_pos q (NOLOCK) JOIN (
						SELECT [date] recon_business_date FROM [get_dates_in_range](@report_date_start,@report_date_end)
					)rec
				ON
					rec.recon_business_date = q.recon_business_date
				left JOIN tbl_merchant_category m (NOLOCK)
	...
	WHERE 			
				--q.post_tran_cust_id >= @rpt_tran_id1 AND
				q.tran_completed = 1
				AND
	...
	IF @@ROWCOUNT = 0
		INSERT INTO @report_result2 (Warning, StartDate, EndDate) VALUES ('No transactions.', @StartDate, @EndDate)			
...
'P','0.0001','0','0','M',TT.MERCHANT_ID,TT.PAYEE,'RECOUPDATA','RECOUPDATA','00','00','0200','20150717',
 ...
 'RECOUPDATA','Approved','2','NGN','Naira','1','0','0','0','0','0','0','1','0',ACCOUNT_NO,'1',
