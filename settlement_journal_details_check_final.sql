			 ...
			 DROP TABLE settle_tran_details_tab_20170423

			 USE [postilion_office]
				GO
				CREATE NONCLUSTERED INDEX [ix_temp_journal_data_post_tran]
				ON [dbo].[temp_journal_data] ([post_tran_id])
			  ...
			  trxn_amount=ISNULL(J.amount,0),
			 	...
			 	trxn_date=j.business_date,
			       ...
			       ,pt.[PTC_pan_reference]
			 													...
			 													INTO settle_tran_details_tab_20170423
			 														 ...
			 														 (select  [adj_id]
			 															  ...
			 															  ,[coa_description]
			 															  ,[coa_type]
			 															  ,[coa_config_state] from  temp_journal_data (NOLOCK)   )J
			 																			 JOIN 
			 												...
			 												LEFT   JOIN 
			 								  (SELECT  PT_post_tran_id,PT_post_tran_cust_id,ptc_terminal_id,PT_tran_nr, PT_retention_data FROM temp_post_tran_data (NOLOCK) WHERE PT_tran_postilion_originated =1)PTT 
			 																ON
			 		...
			 		CREATE NONCLUSTERED INDEX [ix_settle_tran_details_tab_20170423_1]
			 ON [dbo].[settle_tran_details_tab_20170423] ([source_node_name])
			 INCLUDE ([Unique_key])
			 GO
			 														...
			 														DELETE FROM final_settle_tran_details_tab_20170423
			 														  WHERE index_no IN (SELECT index_no FROM final_settle_tran_details_tab_20170423 (NOLOCK) where  (source_node_name 
			 														 IN ('SWTNCS2src','SWTSHOPRTsrc','SWTNCSKIMsrc','SWTNCSKI2src','SWTFBPsrc','SWTUBAsrc'
			 														 ,'SWTZIBsrc','SWTPLATsrc') and ptid IN (select ptid from final_settle_tran_details_tab_20170423  (NOLOCK) where source_node_name in ('SWTASPPOSsrc','SWTASGTVLsrc')
			 														)))
			 
			 
			 USE [postilion_office]
			 GO
			 CREATE NONCLUSTERED INDEX [ix_settle_tran_details_tab_20170423_2]
			 ON [dbo].[settle_tran_details_tab_20170423] ([index_no])
			 ...
			 USE [postilion_office]
			 GO
			 CREATE NONCLUSTERED INDEX [ix_settle_tran_details_tab_20170423_3]
			 ON [dbo].[settle_tran_details_tab_20170423] ([Unique_key],[source_node_name])
			 INCLUDE ([index_no])												  
			 														  
			 drop table final_settle_tran_details_tab_20170423														  
			 SELEct   
			 Acquirer
			 ,PT.bank_code
			 ,card_type
			 ...
			 ,pan  pan
			 into  final_settle_tran_details_tab_20170423
			    FROM       settle_tran_details_tab_20170423  pt (nolock)  
			 ...
			 'MASTERCARD POS PURCHASE PTSP BILLING') 
			 	...
			 	drop table temp_breakdown 						
			 		 SELECT BANK_CODE, trxn_category, sum(trxn_amount)	amount	 into temp_breakdown FROM  settlement_summary_breakdown (nolock)
