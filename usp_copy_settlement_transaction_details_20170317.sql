...
/****** Object:  StoredProcedure [dbo].[usp_copy_settlement_transaction_details]    Script Date: 03/17/2017 16:26:37 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_copy_settlement_transaction_details] (@settlement_date NVARCHAR(10), @retention_period INT)  AS
							BEGIN
							 ...
							 BEGIN
									...
									BEGIN      			
														...
														,[proc_online_system_id] FROM   post_tran AS PT1 WITH (NOLOCK, INDEX(ix_post_tran_9))
														   WHERE   post_tran_id  > '+@start_post_tran_id+' AND  recon_business_date = '''+@previous_settlement_date+''') PT
															...
															WHERE LEFT(sink_node_name,2)<> ''SB''
...
EXEC('														
					...
					'); ExeC('	CREATE INDEX ix_settle_tran_details  ON  ##settle_tran_details (
									PTID 
							)
							INSERT INTO ['+@previous_table_name+']
							SELECT report_results.bank_code,report_results.trxn_category,report_results.Debit_account_type,report_results.Credit_account_type,report_results.trxn_amount,report_results.trxn_fee,report_results.trxn_date,report_results.currency,report_results.late_reversal,report_results.card_type,terminal_type,report_results.source_node_name,report_results.Unique_key,report_results.Acquirer,report_results.Issuer,report_results.Volume,report_results.Value_RequestedAmount,report_results.Value_SettleAmount,report_results.ptid,report_results.ptcid,report_results.index_no,
	...
	EXEC('IF (OBJECT_ID(''tempdb.dbo.##settle_tran_details'') IS NOT NULL) BEGIN
		...
		END
			...
			exec('	IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N''[dbo].[settlement_summary_breakdown_details]'')) BEGIN
						SET ANSI_NULLS ON
						SET QUOTED_IDENTIFIER ON	 
						ALTER VIEW settlement_summary_breakdown_details AS SELECT * FROM  ['+@previous_table_name+'] (NOLOCK)
					END
					ELSE  BEGIN
						SET ANSI_NULLS ON
						SET QUOTED_IDENTIFIER ON
						CREATE VIEW settlement_summary_breakdown_details AS SELECT * FROM  ['+@previous_table_name+'](NOLOCK)
					END')
								END
								
								
	DECLARE @temp_post_tran_id BIGINT							
	SELECT    top	1 @temp_post_tran_id = (post_tran_id) FROM SSTL_JOURNAL_TEMP (NOLOCK) WHERE CONVERT(VARCHAR(10), business_date,112)  ='20170314' ORDER BY  entry_id ASC;
    SELECT   TOP  1 post_tran_id from sstl_journal_temp  (NOLOCK) WHERE post_tran_id <@temp_post_tran_id and REPLACE(CONVERT(VARCHAR(10), business_date,112),'-','') <='20170314' ORDER BY  post_tran_id ASC
...
EXEC('				
						...
						,J.credit_acc_id
							...
							,coa.config_state	coa_config_state
							 INTO ##temp_journal_data_local
							FROM	(SELECT  Jr.adj_id,Jr.entry_id,Jr.config_set_id,Jr.session_id,Jr.post_tran_id,Jr.post_tran_cust_id,Jr.sdi_tran_id,Jr.acc_post_id,Jr.nt_fee_acc_post_id,Jr.coa_id,Jr.coa_se_id,Jr.se_id,Jr.amount,Jr.amount_id,Jr.amount_value_id,Jr.fee,Jr.fee_id,Jr.fee_value_id,Jr.nt_fee,Jr.nt_fee_id,Jr.nt_fee_value_id,Jr.debit_acc_nr_id,Jr.debit_acc_id,Jr.debit_cardholder_acc_id,Jr.debit_cardholder_acc_type,Jr.credit_acc_nr_id,Jr.credit_acc_id,Jr.credit_cardholder_acc_id,Jr.credit_cardholder_acc_type,Jr.business_date,Jr.granularity_element,Jr.tag,Jr.spay_session_id,Jr.spst_session_id FROM dbo.[sstl_journal_temp] jr WHERE post_tran_id  > '+@start_post_tran_id+' 	AND business_date = '''+@settlement_date+''') J 
							LEFT OUTER JOIN dbo.sstl_se_acc_nr_w AS DebitAccNr (NOLOCK)
							ON (J.debit_acc_nr_id = DebitAccNr.acc_nr_id AND J.config_set_id = DebitAccNr.config_set_id)
							LEFT OUTER JOIN dbo.sstl_se_acc_nr_w AS CreditAccNr  (NOLOCK)
							ON (J.credit_acc_nr_id = CreditAccNr.acc_nr_id AND J.config_set_id = CreditAccNr.config_set_id)
...
EXEC('
								...
								,source_node_key
								,[proc_online_system_id] FROM   post_tran AS PT1 WITH (NOLOCK, INDEX(ix_post_tran_9))
								   WHERE   post_tran_id  > '+@start_post_tran_id+' AND  recon_business_date = '''+@settlement_date+''') PT
									 LEFT JOIN  Post_tran_cust AS PTC WITH  (NOLOCK, INDEX(pk_post_tran_cust))
									ON (PT.post_tran_cust_id = PTC.post_tran_cust_id 
									 AND  LEFT( source_node_name,2 ) <> ''SB''
									AND  CHARINDEX(''TPP'',source_node_name )<1
...
(SELECT * FROM  tbl_PTSP PTSP (nolock) where  PT.PTC_terminal_id = PTSP.terminal_id)

(SELECT * FROM  tbl_PTSP_Account PA(nolock) on (PTSP.PTSP_code = PA.PTSP_code))



(SELECT * FROM tbl_xls_settlement a (NOLOCK) WHERE a.terminal_id  NOT IN(select terminal_id from tbl_reward_OutOfBand (NOLOCK)) AND (PT.PTC_terminal_id= y.terminal_id  AND PT.PT_retrieval_reference_nr = y.rr_number   
													            AND (-1 * PT.PT_settle_amount_impact)/100 = y.amount
																AND substring (CAST (PT.PT_datetime_req AS VARCHAR(8000)), 1, 10)
																= substring(CAST (y.trans_date AS VARCHAR(8000)), 1, 10))y
													)Y 
(SELECT * FROM tbl_merchant_category m (NOLOCK) WHERE PT.PTC_merchant_type = m.category_code)

(SELECT * FROM  tbl_merchant_category_visa s (NOLOCK) WHERE PT.PTC_merchant_type  = s.category_code )

(SELECT * FROM  tbl_merchant_account a (NOLOCK) JOIN tbl_merchant_category_visa s (NOLOCK) ON PT.PTC_merchant_type  = s.category_code AND PT.PTC_card_acceptor_id_code = a.card_acceptor_id_code  )
 
(SELECT * FROM Reward_Category r (NOLOCK) JOIN  (SELECT * FROM tbl_xls_settlement a (NOLOCK) WHERE a.terminal_id  NOT IN(select terminal_id from tbl_reward_OutOfBand (NOLOCK)) AND (PT.PTC_terminal_id= y.terminal_id  AND PT.PT_retrieval_reference_nr = y.rr_number   
													            AND (-1 * PT.PT_settle_amount_impact)/100 = y.amount
																AND substring (CAST (PT.PT_datetime_req AS VARCHAR(8000)), 1, 10)
																= substring(CAST (y.trans_date AS VARCHAR(8000)), 1, 10))y
													)Y ON y.extended_trans_type = r.reward_code)

(SELECT * FROM  tbl_merchant_category_web mer (NOLOCK) WHERE PT.PTC_merchant_type = mer.category_code )

(SELECT * FROM  tbl_terminal_owner ow ON PT.PTC_terminal_id = ow.terminal_id  )
