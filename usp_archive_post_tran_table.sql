CREATE PROCEDURE dbo.usp_archive_post_tran_table AS 
...
BEGIN
			...
			DECLARE @post_tran_id_range_check BIGINT
					...
					FROM [postilion_office_old].[dbo].post_tran_archive_sources with  (nolock)
					WHERE  
			...
			IF(@archive_id  IS NOT NULL ) BEGIN
						...
						DECLARE @session_completed INT 
						  ...
						  FROM [postilion_office_old].[dbo].[post_tran_archive_session]  with (NOLOCK)
									...
									drop table ##temp_post_tran_id
										  ...
										  IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id') IS  NULL) BEGIN
												...
												IF (@first_post_tran_id IS NULL OR @first_post_tran_id =0) BEGIN
									 			   					  SET @sqlQuery= '
																		   INSERT INTO  [postilion_office_old].[dbo].[post_tran_archive_session](  [last_datetime_req]
																		  ,[last_post_tran_id]
										   ...
										   DECLARE @max_post_tran_id BIGINT
											...
											COPY_POST_TRAN_1:				
											 IF(   OBJECT_ID('tempdb.dbo.##temp_post_tran_id_2') IS  NOT NULL) BEGIN
											   ...
											   DROP TABLE   ##temp_post_tran_id_2
											END
											set @sqlQuery= ('SELECT   MAX(post_tran_id)  POST_TRAN_ID INTO  ##temp_post_tran_id_2 FROM [dbo].['+@tableName+']  with (nolock) ')
											EXEC(@sqlQuery);

											 IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id_2') IS NOT NULL) BEGIN
											   SELECT   @max_post_tran_id =post_tran_id from  ##temp_post_tran_id_2;
											   DROP TABLE ##temp_post_tran_id_2
											END
											
											
											SET @sqlQuery= ('SELECT  max(post_tran_id) POST_TRAN_ID INTO  ##temp_post_tran_id_3 FROM ['+@serverName+'].['+@database_name+'].dbo.post_tran  WHERE recon_business_date ='''+convert(varchar(30),@startDate, 112)+'')
											EXEC(@sqlQuery);

											 IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id_3') IS NOT NULL) BEGIN
											   SELECT   @post_tran_id_range_check =post_tran_id from  ##temp_post_tran_id_3;
											   DROP TABLE ##temp_post_tran_id_3
											END
											
										
											
										  PRINT  '@max_post_tran_id: '+CONVERT(VARCHAR(MAX), @max_post_tran_id)
											SET @sqlQuery = '
													INSERT INTO [dbo].['+@tableName+'] (
			[post_tran_id],[post_tran_cust_id],[settle_entity_id],[batch_nr],[prev_post_tran_id],[next_post_tran_id],[sink_node_name],[tran_postilion_originated],[tran_completed],[message_type],[tran_type],[tran_nr],[system_trace_audit_nr],[rsp_code_req],
			[rsp_code_rsp],[abort_rsp_code],[auth_id_rsp],[auth_type],[auth_reason],[retention_data],[acquiring_inst_id_code],[message_reason_code],[sponsor_bank],[retrieval_reference_nr],[datetime_tran_gmt],
			[datetime_tran_local],[datetime_req],[datetime_rsp],[realtime_business_date],[recon_business_date],[from_account_type],[to_account_type],[from_account_id],[to_account_id],[tran_amount_req],[tran_amount_rsp],
			[settle_amount_impact],[tran_cash_req],[tran_cash_rsp],[tran_currency_code],[tran_tran_fee_req],[tran_tran_fee_rsp],[tran_tran_fee_currency_code],[tran_proc_fee_req],[tran_proc_fee_rsp],[tran_proc_fee_currency_code],
			[settle_amount_req],[settle_amount_rsp],[settle_cash_req],[settle_cash_rsp],[settle_tran_fee_req],[settle_tran_fee_rsp],[settle_proc_fee_req],[settle_proc_fee_rsp],[settle_currency_code],[pos_entry_mode],[pos_condition_code],[additional_rsp_data],[tran_reversed],[prev_tran_approved],[issuer_network_id],
			[acquirer_network_id],[extended_tran_type],[ucaf_data],[from_account_type_qualifier],[to_account_type_qualifier],[bank_details],[payee],[card_verification_result],[online_system_id],[participant_id],[receiving_inst_id_code],[routing_type],[pt_pos_operating_environment],[pt_pos_card_input_mode],
			[pt_pos_cardholder_auth_method],[pt_pos_pin_capture_ability],[pt_pos_terminal_operator],[source_node_key],[proc_online_system_id],
			[opp_participant_id],[pos_geographic_data],[payer_account_id])
			SELECT
				[post_tran_id],[post_tran_cust_id],[settle_entity_id],[batch_nr],[prev_post_tran_id],[next_post_tran_id],[sink_node_name],[tran_postilion_originated],[tran_completed],[message_type],[tran_type],[tran_nr],[system_trace_audit_nr],[rsp_code_req],
			[rsp_code_rsp],[abort_rsp_code],[auth_id_rsp],[auth_type],[auth_reason],[retention_data],[acquiring_inst_id_code],[message_reason_code],[sponsor_bank],[retrieval_reference_nr],[datetime_tran_gmt],
			[datetime_tran_local],[datetime_req],[datetime_rsp],[realtime_business_date],[recon_business_date],[from_account_type],[to_account_type],[from_account_id],[to_account_id],[tran_amount_req],[tran_amount_rsp],
			...
			[pt_pos_cardholder_auth_method],[pt_pos_pin_capture_ability],[pt_pos_terminal_operator],[source_node_key],[proc_online_system_id],
			[opp_participant_id],[pos_geographic_data],[payer_account_id]
			 FROM  OPENQUERY (['+@serverName+'], ''SELECT TOP ( '+@batchSize+')
			[post_tran_id],[post_tran_cust_id],[settle_entity_id],[batch_nr],[prev_post_tran_id],[next_post_tran_id],[sink_node_name],[tran_postilion_originated],[tran_completed],[message_type],[tran_type],[tran_nr],[system_trace_audit_nr],[rsp_code_req],
			[rsp_code_rsp],[abort_rsp_code],[auth_id_rsp],[auth_type],[auth_reason],[retention_data],[acquiring_inst_id_code],[message_reason_code],[sponsor_bank],[retrieval_reference_nr],[datetime_tran_gmt],
			[datetime_tran_local],[datetime_req],[datetime_rsp],[realtime_business_date],[recon_business_date],[from_account_type],[to_account_type],[from_account_id],[to_account_id],[tran_amount_req],[tran_amount_rsp],
			[settle_amount_impact],[tran_cash_req],[tran_cash_rsp],[tran_currency_code],[tran_tran_fee_req],[tran_tran_fee_rsp],[tran_tran_fee_currency_code],[tran_proc_fee_req],[tran_proc_fee_rsp],[tran_proc_fee_currency_code],
			...
			[pt_pos_cardholder_auth_method],[pt_pos_pin_capture_ability],[pt_pos_terminal_operator],[source_node_key],[proc_online_system_id],
			[opp_participant_id],[pos_geographic_data],[payer_account_id]
			FROM  ['+@database_name+'].dbo.post_tran with  (nolock, INDEX(ix_post_tran_1)) WHERE   post_tran_id > '+ convert(VARCHAR(20), @max_post_tran_id)+' AND post_tran_id <= '+convert(VARCHAR(20),@post_tran_id_range_check)+' ORDER BY post_tran_id ;'')';
			print @sqlQuery;
			exec (@sqlQuery);

												IF @@ROWCOUNT >0 GOTO COPY_POST_TRAN_1
											SET ROWCOUNT 0
												...
												EXEC ('SELECT COUNT(recon_business_date) rec_count  INTO ##archive_day_count FROM  [dbo].['+@tableName+']  WHERE recon_business_date  ='''+@startDate+'''')
												...
												set @sqlQuery = ('DECLARE @max_post_tran_id BIGINT;
																  SELECT @max_post_tran_id  = MAX(post_tran_id) FROM [dbo].['+@tableName+'] with (NOLOCK);
																  SELECT   
																		last_datetime_req            
																		,last_post_tran_id     
																		,last_post_tran_cust_id      
																...
																UPDATE  [postilion_office_old].[dbo].[post_tran_archive_session] SET 
																...
																FROM 
																   [postilion_office_old].[dbo].[post_tran_archive_session] sess
																JOIN
			...
			[opp_participant_id],[pos_geographic_data],[payer_account_id]
												   ...
												   WHERE  recon_business_date =  CONVERT(DATETIME,'''+CONVERT(VARCHAR(30),@startDate, 112)+''') AND
													post_tran_id NOT IN  (SELECT post_tran_id FROM [dbo].['+@tableName+'] with  (NOLOCK)  WHERE recon_business_date = '''+CONVERT(VARCHAR(30),@startDate, 112)+''');
										 ...
										 END
											   ...
											   END
								   					  SET @sqlQuery= ' UPDATE  [postilion_office_old].[dbo].[post_tran_archive_session] SET session_completed =1 WHERE  [session_id]='+@session_id+';
															 INSERT INTO  [postilion_office_old].[dbo].[post_tran_archive_session] (  [last_datetime_req]
									@sqlQuery									  ,[last_post_tran_id]
																		  ,[last_post_tran_cust_id]
																		  ,[last_tran_nr]
																		  ,[last_retrieval_reference_nr]
																		  ,[last_system_trace_audit_nr]
						...
						SELECT  @local_total_post_tran_id = COUNT (post_tran_id) FROM post_tran  with (NOLOCK) WHERE recon_business_date BETWEEN  '''+ CONVERT(varchar(10), @startDate, 112)+''' AND    '''+CONVERT(varchar(10), @endDate, 112)+''';
						SELECT  @local_total_post_tran_cust_id = COUNT (post_tran_cust_id) FROM post_tran with (NOLOCK) WHERE recon_business_date BETWEEN  '''+ CONVERT(varchar(10), @startDate, 112)+''' AND    '''+CONVERT(varchar(10), @endDate, 112)+''';
						IF(
