	...
	GO
	/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 04/05/2016 10:01:16 ******/
	SET ANSI_NULLS ON
	GO
				...
				END
					...
					WHILE (DATEDIFF(MINUTE,@datetime_start, @datetime_stop)>0) 
						BEGIN
							...
							SELECT @datetime_start = MIN(datetime_req) FROM post_tran WITH(NOLOCK);
							SET @datetime_stop= DATEADD(MINUTE,@period_interval,@datetime_start);
						
							
							SELECT @first_post_tran_id = min (post_tran_id), @last_post_tran_id = max (post_tran_id) FROM post_tran (NOLOCK) WHERE  datetime_req <=@datetime_stop  OPTION (MAXDOP 12)
							
							SELECT @first_post_tran_cust_id = min (post_tran_cust_id), @last_post_tran_cust_id = max (post_tran_cust_id) FROM post_tran (NOLOCK) WHERE 
							(post_tran_id BETWEEN  @first_post_tran_id AND   @last_post_tran_id )
							...
							BEGIN TRY
									...
									DELETE FROM extract_tran WHERE post_tran_id BETWEEN  @first_post_tran_id AND   @last_post_tran_id  OPTION(MAXDOP 12);
									
																	
									PRINT 'Cleaning post_tran records older than: '+CONVERT(VARCHAR(30),@datetime_stop)
									SET ROWCOUNT @batch_size
									DELETE_POST_TRAN:
									DELETE FROM post_tran  WHERE   post_tran_id  BETWEEN  @first_post_tran_id AND   @last_post_tran_id  OPTION(MAXDOP 12);
									...
									DELETE FROM post_tran_cust  WHERE   post_tran_cust_id   BETWEEN  @first_post_tran_cust_id AND    @last_post_tran_cust_id AND post_tran_cust_id  NOT IN (
							            			 SELECT post_tran_cust_id FROM post_tran (NOLOCK) where datetime_req > @datetime_stop
										)  OPTION(MAXDOP 12);
