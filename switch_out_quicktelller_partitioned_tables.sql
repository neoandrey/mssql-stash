...
SET @part_number  = DATEDIFF(M,@base_date, @start_date);
...
ALTER  index INDEX_transactions_ON_RequestReference ON Transactions_TEMP DISABLE
ALTER  index INDEX_transactions_temp_ON_ResponseCode ON Transactions_TEMP DISABLE
ALTER  index ix_Transaction_ON_ID ON Transactions_TEMP DISABLE
ALTER  index IX_transactions_ON_PaymentDate_2 ON Transactions_TEMP DISABLE
...
CREATE TABLE [dbo].[Transactions_temp_'+@month_ext+'](
...
(
 ...
 EXEC('ALTER TABLE TRANSACTIONS_TEMP  SWITCH PARTITION '+@part_index+' TO [TRANSACTIONS_TEMP_'+@month_ext+']')
 ...
 ALTER  index INDEX_transactions_ON_RequestReference ON Transactions_TEMP REBUILD WITH (online = ON )
ALTER  index INDEX_transactions_temp_ON_ResponseCode ON Transactions_TEMP REBUILD WITH (online = ON )
ALTER  index ix_Transaction_ON_ID ON Transactions_TEMP REBUILD WITH (online = ON )
ALTER  index IX_transactions_ON_PaymentDate_2 ON Transactions_TEMP REBUILD WITH (online = ON )
