...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown]    Script Date: 05/08/2016 17:27:22 ******/
SET ANSI_NULLS ON
GO
...
SELECT TOP 1  (cast (J.business_date as varchar(40)))
	   ...
	   ON j.post_tran_id = PT.post_tran_id  and (J.business_date >= @from_date AND J.business_date <= (@to_date))
	...
	SELECT @first_post_tran_id = MIN(post_tran_id ) FROM sstl_journal_all(NOLOCK) where business_date >=@from_date and business_date <=@to_date 
	SELECT @last_post_tran_id =  max(post_tran_id ) FROM sstl_journal_all(NOLOCK) where business_date >=@from_date and business_date <=@to_date 
	SELECT @first_post_tran_cust_id= MIN(post_tran_cust_id) FROM sstl_journal_all (NOLOCK) where business_date >=@from_date and business_date <=@to_date 
	SELECT @last_post_tran_cust_id=  max(post_tran_cust_id) FROM sstl_journal_all (NOLOCK) where business_date >=@from_date and business_date <=@to_date 
END
ELSE BEGIN
SELECT @first_post_tran_id = MIN(post_tran_id ) FROM sstl_journal_all(NOLOCK) where business_date >=@from_date and business_date <=@to_date 
SELECT @last_post_tran_id =  max(post_tran_id ) FROM sstl_journal_all(NOLOCK) where business_date >=@from_date and business_date <=@to_date 
...
SELECT   [post_tran_id]
      ...
      ,[source_node_key]
      ,[proc_online_system_id]
  FROM [postilion_office].[dbo].[post_tran] t (NOLOCK, index(ix_post_tran_1)) 
  JOIN (SELECT part rsp_code_rsp FROM usf_split_string('00,11,09', ',')) res
  ON   t.rsp_code_rsp = res.rsp_code_rsp   
WHERE 
...
DELETE FROM [#post_tran_temp]
WHERE 
	      post_tran_id  IN(
		  
		  SELECT t.post_tran_id FROM [#post_tran_temp] t JOIN tbl_late_reversals ll (NOLOCK) 
		  ON t.post_tran_id = ll.post_tran_id
		   WHERE 
		   ll.recon_business_date >=  REPLACE(CONVERT(VARCHAR(10), @report_date_start,111),'/', '') AND  ll.recon_business_date <=  REPLACE(CONVERT(VARCHAR(10), GETDATE(),111),'/', '')
...
SELECT  [post_tran_cust_id]
      ...
      ,[pan_encrypted]
      ,[pan_reference]
  FROM [postilion_office].[dbo].[post_tran_cust] (NOLOCK,INDEX(pk_post_tran_cust))
  WHERE 
  post_tran_cust_id IN (SELECT post_tran_cust_id FROM [#post_tran_temp]) 
  AND 
...
END,
	trxn_category=CASE WHEN (PT.tran_type ='01')  
                           ...
                           WHEN (dbo.fn_rpt_isBillpayment (PTC.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,ptc.card_acceptor_id_code ,ptc.source_node_name,pt.tran_type,ptc.pan) = '1' 
                           and (DebitAccNr.acc_nr like '%SVA_FEE_RECEIVABLE' or CreditAccNr.acc_nr like '%SVA_FEE_RECEIVABLE') ) 
                           AND dbo.fn_rpt_isBillpayment_IFIS(ptc.terminal_id) = 1  then 'BILLPAYMENT IFIS REMITTANCE'
...
acc.bank_code1, acc.bank_code, PT.acquiring_inst_id_code,pt.extended_tran_type,PTC.merchant_type, dbo.fn_rpt_isBillpayment_IFIS(ptc.terminal_id)
OPTION(RECOMPILE)
