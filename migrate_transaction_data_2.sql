...
GO
/****** Object:  StoredProcedure [dbo].[migrate_transaction_data]    Script Date: 07/04/2014 15:22:58 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER  PROCEDURE [dbo].[migrate_transaction_data]  @trans_server VARCHAR(50), @trans_date VARCHAR(30) 
...
set quoted_identifier off
		...
		IF  (OBJECT_ID('tempdb.dbo.##global_transaction_table') IS NOT NULL)
		BEGIN
		DROP TABLE tempdb.dbo.##global_transaction_table
		END
		
				IF  (OBJECT_ID('tempdb.dbo.##TEMP_POST_TRAN') IS NOT NULL)
		BEGIN
		DROP TABLE tempdb.dbo.##TEMP_POST_TRAN
		END


		IF  (OBJECT_ID('tempdb.dbo.##TEMP_POST_TRAN_CUST') IS NOT NULL)
		BEGIN
		DROP TABLE tempdb.dbo.##TEMP_POST_TRAN_CUST
		END
