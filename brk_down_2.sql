...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown]    Script Date: 05/09/2016 12:02:15 ******/
SET ANSI_NULLS ON
GO
...
FROM   post_tran AS PT (NOLOCK)
 LEFT JOIN  Post_tran_cust AS PTC (NOLOCK, INDEX(pk_post_tran_cust))
...
USE [postilion_office];  CREATE INDEX [ix_temp_post_tran_data_12]    ON [dbo].[temp_post_tran_data]    ([PT_tran_postilion_originated])      INCLUDE ([PT_post_tran_cust_id], [PT_tran_nr], [PT_retention_data])      WITH (FILLFACTOR=90)
USE [postilion_office];  CREATE INDEX [ix_temp_post_tran_data_13]    ON [dbo].[temp_post_tran_data]    ([PT_post_tran_cust_id], [PT_tran_postilion_originated], [PT_tran_nr])      INCLUDE ([PT_retention_data])      WITH (FILLFACTOR=90)
USE [postilion_office];  CREATE INDEX [ix_temp_post_tran_data_14]    ON [dbo]    .[temp_post_tran_data]([PT_tran_postilion_originated], [PTC_totals_group])      INCLUDE ([PT_post_tran_id], [PT_post_tran_cust_id], [PT_sink_node_name], [PT_message_type], [PT_tran_type], [PT_tran_nr], [PT_system_trace_audit_nr], [PT_acquiring_inst_id_code], [PT_retrieval_reference_nr], [PT_settle_amount_impact], [PT_settle_amount_rsp], [PT_settle_currency_code], [PT_tran_reversed], [PT_extended_tran_type], [PT_payee], [PTC_source_node_name], [PTC_pan], [PTC_terminal_id], [PTC_card_acceptor_id_code], [PTC_merchant_type], [PTC_card_acceptor_name_loc])      WITH (FILLFACTOR=70, ONLINE=ON)
