...
SET @enddate = ISNULL( @enddate, CONVERT(VARCHAR(30),(DATEADD (dd, 0, GetDate())), 112))
 ...
 SP_HELP tbl_cardless_reversals
 SELECT   id = IDENTITY(INT,1,1), *  into tbl_cardless_reversals   FROM(
  SELECT  t.pan AS pan,
                  t.source_node_key,
                  t.terminal_id,
            t.source_node_name, 
...
t.from_account_id,
  ...
  FROM  post_tran_summary t (NOLOCK) LEFT  JOIN
            post_tran_summary tt (nolock) on (t.post_tran_id = tt.post_tran_id and
