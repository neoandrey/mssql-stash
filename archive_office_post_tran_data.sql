USE [post_office_mega32]
GO
/****** Object:  StoredProcedure [dbo].[archive_office_post_tran_data]    Script Date: 11/10/2014 09:51:38 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[archive_office_post_tran_data] (@batch_size BIGINT)
AS
BEGIN
...
SELECT @min_post_tran_cust_id =MIN(post_tran_cust_id) FROM  [172.25.10.8].[postilion_office].[dbo].[post_tran_cust] 

SELECT @start_post_tran_cust_id =( SELECT  TOP 1 post_tran_cust_id FROM post_tran_cust (NOLOCK) ORDER BY post_tran_cust_id DESC);
...
FROM [172.25.10.8].[postilion_office].[dbo].[post_tran_cust]
WHERE post_tran_cust_id >@start_post_tran_cust_id AND  post_tran_cust_id<=@end_post_tran_cust_id


SELECT @min_post_tran_id =(SELECT TOP 1 post_tran_id FROM  [172.25.10.8].[postilion_office].[dbo].[post_tran] ORDER BY post_tran_id ASC)

SET @start_post_tran_id =( SELECT  TOP 1 post_tran_id FROM post_tran (NOLOCK) ORDER BY post_tran_id DESC);
...
FROM [172.25.10.8].[postilion_office].[dbo].[post_tran] 
WHERE 

post_tran_id >@start_post_tran_id AND  post_tran_id<=@end_post_tran_id
