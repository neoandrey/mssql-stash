...
END,
	trxn_category=CASE WHEN (PT.PT_tran_type ='01')  
                           ...
                           WHEN (dbo.fn_rpt_transfers_sett(PT.PTC_terminal_id,PT.PT_payee,PT.PTC_card_acceptor_name_loc,
                                 ...
                                 and (PT.PT_tran_type in ('50') or(dbo.fn_rpt_autopay_intra_sett (PT.PT_tran_type,PT.PTC_source_node_name) = 1))
...
END,
	...
	business_date=j.business_date,
        ...
        Acquirer = (case when (not ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) )) then ''
                      when ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) ) AND (PT.acc_acquirer_inst_id1 = PT.PT_acquiring_inst_id_code or PT.acc_acquirer_inst_id2 = PT.PT_acquiring_inst_id_code or PT.acc_acquirer_inst_id3 = PT.PT_acquiring_inst_id_code or PT.acc_acquirer_inst_id4 = PT.PT_acquiring_inst_id_code or PT.acc_acquirer_inst_id5 = PT.PT_acquiring_inst_id_code) then PT.acc_bank_code 
        ...
        Issuer = (case when (not ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) )) then ''
                      when ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) ) and (substring(PT.PTC_totals_group,1,3) = PT.acc_bank_code1) then PT.acc_bank_code
                      else substring(PT.PTC_totals_group,1,3) END)
                      into #report_results FROM
                     #temp_journal_data  J (NOLOCK)
                     join 
                     #post_tran_data PT (NOLOCK )
                    ON (J.post_tran_id = PT.PT_post_tran_id AND J.post_tran_cust_id = PT.PT_post_tran_cust_id)
                    left JOIN 
                    #post_tran_data PTT(NOLOCK )on (PT.PT_post_tran_cust_id = PTT.PT_post_tran_cust_id and PTT.PT_tran_postilion_originated = 1
...
dbo.fn_rpt_autopay_intra_sett (PT.PT_tran_type,PT.PTC_source_node_name),
...
(case when (not ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) )) then ''
                      when ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) ) and (substring(PT.PTC_totals_group,1,3) = PT.acc_bank_code1) then PT.acc_bank_code1
                      else substring(PT.PTC_totals_group,1,3) END),
PT.acc_bank_code1, PT.acc_bank_code, PT.PT_acquiring_inst_id_code,PT.PT_extended_tran_type,PT.PTC_merchant_type, dbo.fn_rpt_isBillpayment_IFIS(PT.PTC_terminal_id)
