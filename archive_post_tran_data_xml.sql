...
IF(@archive_id  IS NOT NULL ) BEGIN
			...
			DECLARE @session_completed INT 
			  ...
			  WHILE (@@FETCH_STATUS=0) BEGIN
				...
				WHILE (DATEDIFF(D,@startDate, @endDate)>0) BEGIN
							  ...
							  IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id') IS  NULL) BEGIN
							   ...
							   DECLARE @max_post_tran_id BIGINT
								...
								COPY_POST_TRAN_XML_1:				
								set @sqlQuery= ('SELECT   ISNULL( MAX(post_tran_id),0)  POST_TRAN_ID INTO  ##temp_post_tran_id_2 FROM ['+@tableName+'] (nolock) WHERE  recon_business_date =  CONVERT(DATETIME,'''+CONVERT(VARCHAR(30),@startDate, 112)+''')  ')
								EXEC(@sqlQuery);
							  ...
							  PRINT  '@max_post_tran_id: '+CONVERT(VARCHAR(MAX), @max_post_tran_id)
								...
								[retrieval_reference_nr] ,
								[recon_business_date],
								CASE WHEN icc_data_req IS not null THEN master.dbo.fn_rpt_archive_icc_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''H:\postilon_office_xml_data\icc_data_req'', REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), icc_data_req)  
								else NULL END icc_data_req
								,CASE WHEN icc_data_rsp IS not null then master.dbo.fn_rpt_archive_icc_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''H:\postilon_office_xml_data\icc_data_rsp'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), icc_data_req)  
								else NULL  END icc_data_rsp
								,CASE WHEN structured_data_req IS not null THEN  master.dbo.fn_rpt_archive_structured_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''H:\postilon_office_xml_data\structured_data_req'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), structured_data_req)  
								ELSE NULL END structured_data_req
								,CASE WHEN structured_data_rsp IS not null THEN master.dbo.fn_rpt_archive_structured_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''H:\postilon_office_xml_data\structured_data_rsp'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''),  structured_data_rsp)   
								...
								FROM  ['+@database_name+'].dbo.post_tran (nolock, INDEX(ix_post_tran_1)) WHERE 	
								   post_tran_id >'+ convert(VARCHAR(20), @max_post_tran_id)+';
								
								'')';
								print @sqlQuery;
								exec (@sqlQuery);

									IF @@ROWCOUNT >0 GOTO COPY_POST_TRAN_XML_1
								SET ROWCOUNT 0
				...
				structured_data_rsp,
				@@SERVERNAME [server_name]
				FROM  ['+@database_name+'].dbo.post_tran (nolock, INDEX(ix_post_tran_9)) WHERE 	
				recon_business_date =  CONVERT(DATETIME,'+CONVERT(VARCHAR(30),@startDate, 112)+') ORDER BY post_tran_id'')
		   WHERE 
		   post_tran_id 
		   NOT IN 
		   ( SELECT post_tran_id FROM ['+@tableName+'] (nolock) WHERE  recon_business_date =  CONVERT(DATETIME,'''+CONVERT(VARCHAR(30),@startDate, 112)+'''))
		    OPTION (MAXDOP 8)
