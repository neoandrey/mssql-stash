...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 02/12/2016 07:57:13 ******/
SET ANSI_NULLS ON
GO
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
    ...
    IF(DATEDIFF(MINUTE, @min_date_time, GETDATE()) >@retention_period_minutes)BEGIN
			CREATE TABLE #temp_post_tran(post_tran_id BIGINT,post_tran_cust_id BIGINT)
		
CREATE CLUSTERED INDEX ix_post_tran_id ON #temp_post_tran(
post_tran_id
)
CREATE NONCLUSTERED INDEX ix_post_tran_cust_id ON #temp_post_tran(
post_tran_cust_id
)
			...
			END
				...
				WHILE (DATEDIFF(MINUTE,@datetime_start, @datetime_stop)>0) 
					BEGIN
						...
						INSERT INTO #temp_post_tran (post_tran_id, post_tran_cust_id)  SELECT post_tran_id, post_tran_cust_id FROM post_tran (NOLOCK) WHERE  datetime_req <=@datetime_stop  OPTION (MAXDOP 8)
					
/*AND post_tran_cust_id  NOT IN (
							SELECT post_tran_cust_id FROM post_tran (NOLOCK, INDEX(ix_post_tran_7)) where datetime_req > @datetime_stop and datetime_req <=  REPLACE(CONVERT(VARCHAR(10), GETDATE(),111),'/', '')
						)
						*/
...
select  @min_post_tran_id =  MIN(post_tran_id) FROM #temp_post_tran
select  @max_post_tran_id =  MAX(post_tran_id) FROM #temp_post_tran
						
						PRINT 'Cleaning extract_tran records older than: '+CONVERT(VARCHAR(30),@datetime_stop)
						BEGIN TRY
						
								DELETE FROM extract_tran WHERE post_tran_id   between  @min_post_tran_id and  @max_post_tran_id OPTION(MAXDOP 12);
								
																
								PRINT 'Cleaning post_tran records older than: '+CONVERT(VARCHAR(30),@datetime_stop)
								SET ROWCOUNT @batch_size
								DELETE_POST_TRAN:
								DELETE FROM post_tran  WHERE   post_tran_id  between  @min_post_tran_id and  @max_post_tran_id OPTION(MAXDOP 12);
								...
								DELETE FROM post_tran_cust  WHERE   post_tran_cust_id  IN (SELECT post_tran_cust_id  FROM #temp_post_tran)  OPTION(MAXDOP 12);
...
dbcc shrinkfile(post_office_log_3, 0)
						...
						PRINT 'Cleaning post_tran_cust records older than: '+CONVERT(VARCHAR(30),@datetime_stop)
								...
								DELETE FROM post_tran_cust  WHERE   post_tran_cust_id  IN (SELECT post_tran_cust_id  FROM #temp_post_tran)  OPTION(MAXDOP 8);
					...
					END
						...
						DELETE FROM #temp_post_tran 
						INSERT INTO #temp_post_tran (post_tran_id, post_tran_cust_id)  SELECT post_tran_id, post_tran_cust_id  FROM post_tran (NOLOCK, INDEX(ix_post_tran_7)) WHERE  datetime_req <=@datetime_stop  OPTION (MAXDOP 12)
						...
						BEGIN TRY
							...
							dbcc shrinkfile(post_office_log, 0)
							dbcc shrinkfile(post_office_log_2, 0)
							dbcc shrinkfile(post_office_log_3, 0)						END TRY
						BEGIN CATCH
						PRINT 'Cleaning post_tran_cust records older than: '+CONVERT(VARCHAR(30),@endDate)
							SET ROWCOUNT @batch_size
						   ...
						   SET ROWCOUNT @batch_size
							...
							DELETE FROM post_tran  WHERE   post_tran_id  IN (SELECT post_tran_id  FROM #temp_post_tran)  OPTION(MAXDOP 8);
