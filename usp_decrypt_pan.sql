CREATE PROCEDURE usp_decrypt_pan @pan_list VARCHAR (8000) , @pan_encrypted_list VARCHAR (8000) 
AS BEGIN
	...
	DEALLOCATE pan_cursor
	       ...
	       DECLARE pan_cursor CURSOR LOCAL FORWARD_ONLY STATIC READ_ONLY FOR SELECT pan,pan_encrypted,pan_index FROM #temp_pan_data;
	       		...
	       		BEGIN
	       		   SELECT * FROM #temp_pan_data WHERE pan_encrypted IN (SELECT part FROM usf_split_string(@pan_encrypted_list, ','));
	       		END
			ELSE
