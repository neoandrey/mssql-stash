...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 11/01/2016 13:37:13 ******/
SET ANSI_NULLS ON
GO
   ...
   INSERT INTO @recon_business_date_table  SELECT [DATE] FROM dbo.get_dates_in_range(@datetime_start, @last_day_to_clean);  
    ...
    WHILE (DATEDIFF(day,@current_clean_date, @last_day_to_clean)>=0) BEGIN  
      ...
      BEGIN TRY  
        ...
        DELETE_EXTRACT_TRAN:  
        DELETE  extract_tran FROM extract_tran  WHERE post_tran_id IN  

   (  SELECT  TOP (@batch_size) post_tran_id   FROM post_tran t3  WITH  (NOLOCK, index(ix_post_tran_7)) WHERE 
    CONVERT(VARCHAR(10), datetime_req,112) = CONVERT(VARCHAR(10), @current_clean_date ,112) ORDER BY datetime_req 
   
     )  OPTION (recompile)
	

         
        IF @@ROWCOUNT >0 GOTO DELETE_EXTRACT_TRAN  
        SET ROWCOUNT 0  
        ...
        SET ROWCOUNT @batch_size  
        DELETE_POST_TRAN:  
        DELETE   FROM post_tran  WHERE  post_tran_id IN  (SELECT top (@batch_size) post_tran_id FROM post_tran WITH (nolock, INDEX(ix_post_tran_7)) WHERE  CONVERT(VARCHAR(10), datetime_req,112) = CONVERT(VARCHAR(10), @current_clean_date ,112) )OPTION (recompile)
        ...
        DELETE FROM dbo.post_tran_cust   WHERE   post_tran_cust_id IN (SELECT TOP (@batch_size) post_tran_cust_id FROM post_tran_cust (NOLOCK, INDEX(pk_post_tran_cust)) WHERE post_tran_cust_id NOT IN (SELECT post_tran_cust_id FROM post_tran (NOLOCK, INDEX(ix_post_tran_2)) where  RECON_BUSINESS_DATE =@current_clean_date  ) order by post_tran_cust_id) OPTION (recompile)
      ...
      PRINT 'Cleaning post_tran_cust records older than: '+CONVERT(VARCHAR(30),@current_clean_date)  
       ...
       -- select @first_post_tran_cust_id = MIN(post_tran_cust_id), @last_post_tran_cust_id = MAX(post_tran_cust_id) FROM post_tran (NOLOCK, INDEX(ix_post_tran_9)) WHERE recon_business_date =@current_clean_date ;  
        ...
        DELETE FROM dbo.post_tran_cust   WHERE   post_tran_cust_id IN (SELECT TOP (@batch_size) post_tran_cust_id FROM post_tran_cust (NOLOCK, INDEX(pk_post_tran_cust)) WHERE post_tran_cust_id NOT IN (select post_tran_cust_id FROM post_tran (NOLOCK, INDEX(ix_post_tran_2))))    OPTION (recompile)
       ...
       -- DELETE FROM dbo.post_tran   WHERE   post_tran_id in (SELECT  TOP (@batch_size)  post_tran_id FROM post_tran (NOLOCK, index(ix_post_tran_9)) WHERE datetime_req >= (SELECT MIN(datetime_req)  FROM post_tran ( NOLOCK)) AND   (CONVERT(VARCHAR(10), datetime_req,112)) <=  (CONVERT(VARCHAR(10),@current_clean_date,112))  )  ;  
        DELETE   FROM post_tran  WHERE  post_tran_id IN  (SELECT top (@batch_size) post_tran_id FROM post_tran WITH (nolock, INDEX(ix_post_tran_7)) WHERE  CONVERT(VARCHAR(10), datetime_req,112) = CONVERT(VARCHAR(10), @current_clean_date ,112) )OPTION (recompile)
