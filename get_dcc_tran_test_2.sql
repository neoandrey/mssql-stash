...
BEGIN
    ...
    SELECT
     trans.datetime_req, cust.pan,terminal_id, retrieval_reference_nr,system_trace_audit_nr, auth_id_rsp, message_type, settle_amount_req, dbo.formatAmount(tran_amount_req,tran_currency_code) base_transaction_amount, dbo.formatAmount(tran_tran_fee_req,tran_currency_code) base_transaction_fee,tran_currency_code AS base_currency_code,dbo.formatAmount(settle_amount_impact,settle_currency_code) scheme_settlement_amount, dbo.formatAmount(settle_amount_req,settle_currency_code) settlement_amount_req, dbo.formatAmount(settle_tran_fee_req,settle_currency_code)settle_tran_fee_feq, settle_currency_code scheme_settelement_currency,  tran_currency_code, message_type,cust.terminal_id, cust.card_acceptor_id_code,trans.tran_type,trans.pos_entry_mode, (SELECT rate FROM post_currencies WHERE currency_code=settle_currency_code)as scheme_settlement_rate  
	...
	WHERE
 	trans.message_type in('0200','0220') AND
		tran_postilion_originated = 0 AND
 ...
 AND
 PATINDEX('%CONV_RATE_SETTLE%', structured_data_req)>0
 AND
		trans.rsp_code_rsp = '00'
		AND tran_reversed = 0
		--AND
				
		--cust.pos_terminal_type IN ('01','02') AND
  ...
  --     and trans.tran_currency_code <> trans.settle_currency_code 
	...
	SELECT * FROM post_tran (NOLOCK) WHERE tran_nr =523071237
