...
DECLARE batch_cursor CURSOR STATIC READ_ONLY LOCAL FORWARD_ONLY FOR SELECT DISTINCT REPLACE(batch_identifier,' ','') FROM #transaction_batch_data;
OPEN batch_cursor;
...
WHILE(@@FETCH_STATUS=0)BEGIN
	...
	SET @batch_sum=0;
	SELECT @batch_sum =SUM(settle_amount_impact) FROM #transaction_batch_data WHERE REPLACE(batch_identifier,' ','') =REPLACE(@batch_identifier,' ','');
	PRINT CHAR(10)+'batch_identifier: '+CONVERT(VARCHAR(50),@batch_identifier)+'; total_settlement_impact: '+CONVERT(VARCHAR(50),@batch_sum);
	UPDATE #transaction_batch_data SET  total_settlement_impact = @batch_sum  WHERE REPLACE(batch_identifier,' ','') =REPLACE(@batch_identifier,' ','');
...
SELECT DISTINCT batch_identifier, retrieval_reference_nr, settle_amount_impact,total_settlement_impact FROM #transaction_batch_data WHERE total_settlement_impact <> 0  ORDER BY batch_identifier
