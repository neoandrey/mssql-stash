if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[osp_rpt_b04_web_pos_acquirer_billing]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[osp_rpt_b04_web_pos_acquirer_billing]
GO
...
CREATE                          PROCEDURE osp_rpt_b04_web_pos_acquirer_billing
		 ...
		 isnull(CASE			
                  ...
                  substring(terminal_id,1,1) as Terminal_type,
                   case when source_node_name like '%MIGS%' then 'MIGS'
                      when source_node_name like 'MGASPV%'
