if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[osp_rpt_b04_WEB_Visa_Remote]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[osp_rpt_b04_WEB_Visa_Remote]
GO
...
CREATE                                                       PROCEDURE osp_rpt_b04_WEB_Visa_Remote
	@StartDate		CHAR(8),	-- yyyymmdd
	@EndDate			CHAR(8),	-- yyyymmdd
	@Totalsbank		VARCHAR(255),
...
SET @EndDate = CONVERT(VARCHAR(30), @report_date_end, 112)
	...
	INSERT INTO @list_of_source_nodes SELECT part FROM usf_split_string(@SourceNodes, ',');
        ...
        DECLARE  @list_of_Totalsbank TABLE(Totalsbank	VARCHAR(13)) 
	
	INSERT INTO  @list_of_Totalsbank SELECT part FROM usf_split_string(@Totalsbank, ',');
	...
	FROM
				...
				left JOIN merchant_category_msc m (NOLOCK, INDEX(PK_merchant_category_msc))
	...
	WHERE 			
				...
				(t.message_type IN ('0220', '0400', '0420')) 
				...
				c.source_node_name IN (SELECT source_node FROM @list_of_source_nodes)
                                AND
				substring(c.totals_group,1,3) IN (SELECT Totalsbank FROM @list_of_Totalsbank)
				--AND (c.terminal_id like '3%')
					
				AND
				sink_node_name NOT IN ('CCLOADsnk','GPRsnk','VTUsnk','VTUSTOCKsnk','PAYDIRECTsnk','SWTMEGAsnk')
                                and  LEFT(c.totals_group,3)<>  'OTH'
