...
ALTER                                                                          PROCEDURE osp_rpt_b04_WEB_Visa_Acquiring
...
SET @EndDate = CONVERT(VARCHAR(30), @report_date_end, 112)
	...
	INSERT INTO  @list_of_source_nodes SELECT part FROM dbo.usf_split_string(@SourceNodes, ',');
        ...
        DECLARE @list_of_AID TABLE(AID	VARCHAR(25)) 
	
	INSERT INTO  @list_of_AID SELECT part FROM dbo.usf_split_string(@AID, ',');
	...
	WHERE 			
				...
				c.source_node_name IN (SELECT source_node FROM @list_of_source_nodes)
                                AND
				t.acquiring_inst_id_code IN (SELECT AID FROM @list_of_AID)
