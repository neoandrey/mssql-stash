CREATE PROCEDURE ebn_failed_transactions_weekends as
BEGIN
	select 
	substring(v.card_number,1,6)+'******'+substring(v.card_number,13,16) PAN,
	...
	CONVERT( VARCHAR(MAX), transmission_date_and_time) as 'Date and Time',
	...
	transaction_amount, substring ((dbo.currency_alpha_code(transaction_currency)),1,3) as TRAN_CURRENCY, 
	a.phone_1,
	a.phone_2,
	a.email,
	reference_number tran_reference_number
	FROM V_autho_activity_view v (NOLOCK)  
	 JOIN
	card c (NOLOCK)
	ON
	...
	and transmission_date_and_time  >= DATEADD(D,-3,REPLACE(CONVERT(VARCHAR(10), GETDATE(),111),'/', ''))
