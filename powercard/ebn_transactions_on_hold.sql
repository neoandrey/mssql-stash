select substr(v.card_number,1,6)||'******'||substr(v.card_number,13,16) PAN, c.embossed_name as Cardholder_Name,(select account_number from accounts_link where entity_id = v.source_account_number) as account_number, source_account_number as shadow_account_number, transmission_date_and_time,external_stan as STAN,
...
card_acc_name_address as LOCATION, transaction_amount, substr ((currency_alpha_code(transaction_currency)),1,3) as TRAN_CURRENCY, reference_number tran_reference_number, matching_date_purge
