...
DECLARE @table_month_suffix_final VARCHAR(10);
		...
		FROM [postilion_office].[dbo].post_tran_archive_sources  (nolock)
		WHERE  
						...
						drop table ##temp_post_tran_id
							  ...
							  IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id') IS  NULL) BEGIN
							   ...
							   DECLARE @max_post_tran_id BIGINT
								...
								COPY_POST_TRAN_1:				
								set @sqlQuery= ('SELECT   ISNULL( MAX(post_tran_id),0)  POST_TRAN_ID INTO  ##temp_post_tran_id_2 FROM ['+@tableName+'] (nolock) ')
								EXEC(@sqlQuery);
...
[pt_pos_cardholder_auth_method],[pt_pos_pin_capture_ability],[pt_pos_terminal_operator],[source_node_key],[proc_online_system_id],
[opp_participant_id],[pos_geographic_data],[payer_account_id]
								FROM  ['+@database_name+'].dbo.post_tran (nolock, INDEX(ix_post_tran_1)) WHERE 	
								   post_tran_id >'+ convert(VARCHAR(20), @max_post_tran_id)+'  ORDER BY post_tran_id ;
