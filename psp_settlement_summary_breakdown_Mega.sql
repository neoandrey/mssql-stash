...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown_Mega]    Script Date: 09/27/2016 08:47:44 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @settle_business_date   varchar(40)
	...
	PT.rsp_code_rsp in ('00','11','09')
	  ...
	  AND (
		 ...
		 or  ((PT.PT_message_type = '0420' and PT.tran_reversed <> 2 ) and ( (PT.PT_settle_amount_impact<> 0 )
		   or (PT.settle_amount_rsp<> 0   and PT.PT_message_type = '0420' and PT.tran_reversed <> 2 and PT.PT_tran_type = 40 and (LEFT(PT.PTC_TERMINAL_ID,1) in ('0','1')))))
	  or (PT.settle_amount_rsp<> 0 and PT.PT_message_type   in ('0200','0220') and PT.PT_tran_type = 40 and (LEFT(PT.PTC_TERMINAL_ID,1) in ('0','1')))
...
SELECT		bank_code = case         
  ...
  CASE 
                          ...
                          AND dbo.fn_rpt_CardType (PT.PTC_PAN ,PT.PT_SINK_NODE_NAME ,PT_TRAN_TYPE,PT.PTC_TERMINAL_ID)= '9'
...
END,
	...
	business_date=j.business_date,
        ...
        terminal_type = PT.PTC_post_terminal_type,--dbo.fn_rpt_terminal_type(PT.PTC_TERMINAL_ID),    
...
dbo.fn_rpt_CardGroup(PT.PTC_PAN), PT.PTC_post_terminal_type,--dbo.fn_rpt_terminal_type(PT.PTC_TERMINAL_ID),
