...
SELECT TOP 1
		...
		FROM postilion_office.dbo.post_tran_archive_sources with  (nolock)
		WHERE  
...
IF(@archive_id  IS NOT NULL ) BEGIN
			...
			DECLARE @session_completed INT 
			  ...
			  FROM [postilion_office].[dbo].[post_tran_xml_archive_session]  with (NOLOCK)
			  ...
			  WHILE (@@FETCH_STATUS=0) BEGIN
				...
				WHILE (DATEDIFF(D,@startDate, @endDate)>0) BEGIN
							  ...
							  IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id') IS  NULL) BEGIN
							   ...
							   DECLARE @max_post_tran_id BIGINT
								...
								set @sqlQuery= ('SELECT   ISNULL( MAX(post_tran_id),0)  POST_TRAN_ID INTO  ##temp_post_tran_id_2 FROM ['+@tableName+'] with (nolock) ')
								EXEC(@sqlQuery);
							  ...
							  PRINT  '@max_post_tran_id: '+CONVERT(VARCHAR(MAX), @max_post_tran_id)
								...
								[structured_data_req],
								[structured_data_rsp] 
								FROM  ['+@database_name+'].dbo.post_tran with  (nolock, INDEX(ix_post_tran_1)) WHERE 	
								...
								SET ROWCOUNT 0
									...
									EXEC ('SELECT COUNT(recon_business_date) rec_count  INTO ##archive_day_count FROM ['+@tableName+']  WHERE recon_business_date  ='''+@startDate+'''')
									...
									set @sqlQuery = ('DECLARE @max_post_tran_id BIGINT;
													  SELECT @max_post_tran_id  = MAX(post_tran_id) FROM [postilion_office].[dbo].['+@tableName+'] with  (NOLOCK);
													  SELECT   
													...
													FROM 
													   [postilion_office].[dbo].[post_tran_xml_archive_session] sess
													JOIN
				...
				[structured_data_req],
				[structured_data_rsp] 
				FROM  ['+@database_name+'].dbo.post_tran with (nolock) WHERE 	
				recon_business_date = ''+ CONVERT(DATETIME,'+CONVERT(VARCHAR(30),@startDate, 112)+')+'' ORDER BY post_tran_id'')
		   WHERE 
		   post_tran_id 
		   NOT IN 
		   ( SELECT post_tran_id FROM ['+@tableName+']  with (nolock) WHERE  recon_business_date =  CONVERT(DATETIME,'''+CONVERT(VARCHAR(30),@startDate, 112)+'''))
		    OPTION (MAXDOP 8)
