			  ...
			  WHILE (@@FETCH_STATUS=0) BEGIN
				...
				WHILE (DATEDIFF(D,@startDate, @endDate)>0) BEGIN
							  ...
							  PRINT  '@max_post_tran_id: '+CONVERT(VARCHAR(MAX), @max_post_tran_id)
								...
								[retrieval_reference_nr] ,
								[recon_business_date],
								CASE WHEN icc_data_req IS not null THEN master.dbo.fn_rpt_archive_icc_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''J:\postilon_office_xml_data\icc_data_req'', REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), icc_data_req)  
								else NULL END icc_data_req
								,CASE WHEN icc_data_rsp IS not null then master.dbo.fn_rpt_archive_icc_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''J:\postilon_office_xml_data\icc_data_rsp'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), icc_data_req)  
								else NULL  END icc_data_rsp
								,CASE WHEN structured_data_req IS not null THEN  master.dbo.fn_rpt_archive_structured_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''J:\postilon_office_xml_data\structured_data_req'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''), structured_data_req)  
								ELSE NULL END structured_data_req
								,CASE WHEN structured_data_rsp IS not null THEN master.dbo.fn_rpt_archive_structured_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  [server_name],''J:\postilon_office_xml_data\structured_data_rsp'',  REPLACE(CONVERT(VARCHAR(10), recon_business_date,112),''/'', ''''),  structured_data_rsp)   
				...
				structured_data_rsp,
				@@SERVERNAME [server_name]
				FROM  ['+@database_name+'].dbo.post_tran (nolock) WHERE 	
