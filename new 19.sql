...
GO
/****** Object:  StoredProcedure [dbo].[usp_get_daily_pos_settlement_data_for_ceaser]    Script Date: 03/23/2017 14:51:04 ******/
SET ANSI_NULLS ON
GO
...
LEFT  JOIN post_currencies cur (NOLOCK) on
...
SELECT c.account_name,account_nr,Acquiring_bank,acquiring_inst_id_code,Addit_Charge,Addit_Party,aggregate_column,amount_cap,Amount_Cap_RD,Authorized_Person,c.bank_code,bearer,card_acceptor_id_code,card_acceptor_name_loc,Category_name,currency_alpha_code,currency_name,Date_Modified,datetime_req,datetime_tran_local,EndDate,extended_tran_type,extended_tran_type_reward,fee_cap,Fee_Cap_RD,Fee_Discount_RD,Fee_type,from_account_type,isDepositTrx,isInquiryTrx,isOtherTrx,isPurchaseTrx,isRefundTrx,isTransferTrx,isWithdrawTrx,merchant_acct_nr,merchant_disc,merchant_type,message_reason_code,message_type,pan,payee,prev_post_tran_id, CASE WHEN PTSP_code= NULL OR  PTSP_code='ISW_' OR ptsp_code  IS NULL  THEN 'ISW_PTSP' ELSE  ptsp_code END ptsp_code,rdm_amount,receiving_inst_id_code,retrieval_reference_nr,Reward_Discount,rsp_code_description,rsp_code_rsp,settle_amount_impact,settle_amount_req,settle_amount_rsp,settle_currency_code,settle_nr_decimals,settle_tran_fee_rsp,sink_node_name,source_node_name,SourceNodeAlias,StartDate,structured_data_req,system_trace_audit_nr,terminal_id,terminal_mode,terminal_owner,to_account_type,tran_cash_req,tran_cash_rsp,tran_currency_code,tran_reversed,tran_tran_fee_rsp,tran_type,tran_type_desciption,TranID,Unique_key,issuer,issuer_alpha_code,issuer_bank_code,acquirer_alpha_code,acquirer_bank_code,acquirer,totals_group,terminal_code,category_code,industry_segment,merchant_receivable,

ISNULL((SELECT top 1 account_no FROM tbl_participant_account iss (nolock) WHERE role = 'ISSUER' AND c.issuer  = iss.participant), '1013015788') issuer_account_no,
ISNULL((SELECT top 1 account_no FROM tbl_participant_account acq (nolock) WHERE role = 'ACQUIRER' AND c.acquirer  = acq.participant), '1013015788') acquirer_account_no,
ISNULL((SELECT top 1 account_name FROM tbl_participant_account term (nolock) WHERE role = 'Terminal Owner' AND c.terminal_code  = term.bank_code), 'ISW_TERMINAL')terminal_owner_name,
ISNULL((SELECT top 1 account_no FROM tbl_participant_account term (nolock) WHERE role = 'Terminal Owner' AND c.terminal_code  = term.bank_code), '1013015788') terminal_owner_account,
ISNULL((SELECT top 1 account_name FROM tbl_participant_account ptsp (nolock) WHERE role = 'PTSP' AND c.ptsp_code  = ptsp.code), 'ISW_PTSP')ptsp_name,
ISNULL((SELECT top 1 account_no FROM tbl_participant_account ptsp (nolock) WHERE role   = 'PTSP' AND c.ptsp_code  = ptsp.code), '1013015788')ptsp_account_number
