...
ALTER                                                        PROCEDURE [dbo].[osp_rpt_b04_web_pos_acquirer_msc3]
...
BEGIN
	...
	(
		...
		msc_value NUMERIC(18,4),
		issuer_fee NUMERIC(18,4)
	)
	...
	DECLARE @issuer_percetange  DECIMAL(10,2);
	DECLARE @msc_value DECIMAL(10,2);
	DECLARE @tran_amount DECIMAL(20,2);
	
	
	SET @card_product = ISNULL(@card_product, '%%');
...
*/    IF (DATEDIFF(D,@report_date_start,@report_date_end )=0)BEGIN
	...
	SELECT
				...
				*/
				...
				auth_id_rsp	,
				account_nr,
				case ((select top 1 country_numeric from mcipm_ip0040t1 (nolock) where LEFT (issuer_acct_range_low,6) = LEFT (c.pan,6))union (select top 1 country_numeric from visa_bin_table (nolock) where LEFT (issuer_acct_range_low,6) = LEFT (c.pan,6))) when '566' then @local_msc else @foreign_msc end as merchant_service_charge,
			     CASE  WHEN (CONVERT(NUMERIC(20,2),dbo.formatAmount(t.tran_amount_rsp, t.tran_currency_code)) * 0.015) <2000 THEN (CONVERT(NUMERIC(20,2),dbo.formatAmount(t.tran_amount_rsp, t.tran_currency_code)) * 0.015)
				       ELSE 2000
				 END AS msc_value,
				 0
	...
	FROM
				post_tran t WITH(NOLOCK) JOIN
				post_tran_cust c WITH(NOLOCK)
				ON  t.post_tran_cust_id = c.post_tran_cust_id
				LEFT JOIN tbl_merchant_account a WITH(NOLOCK)
				ON
	...
	WHERE 			
				...
				AND 
				(t.post_tran_id <= @last_post_tran_id) -- changed from recon_business_date
				AND
	...
	WHERE 			
				...
				--(t.acquiring_inst_id_code = @AcquirerInstId)
				AND
				(card_product <> 'OtherVisaCard' AND  card_product LIKE @card_product)
	...
	IF @@ROWCOUNT = 0
		...
		UPDATE @report_result SET 	issuer_fee = 	(0.25 *msc_value);
