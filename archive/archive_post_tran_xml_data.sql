				...
				WHILE (DATEDIFF(D,@startDate, @endDate)>0) BEGIN
							  ...
							  IF(OBJECT_ID('tempdb.dbo.##temp_post_tran_id') IS  NULL) BEGIN
									...
									IF (@first_post_tran_id IS NULL OR @first_post_tran_id =0) BEGIN
															  ...
															  ,[session_completed]
															  ) VALUES ('''+ CONVERT(VARCHAR(30),@startDate, 112)+''',0, 0, 0, '''', '''','''+ CONVERT(VARCHAR(30),@startDate, 112)+''',0 );';
			                         print @sqlQuery
									 exec( @sqlQuery)
								...
								print  '@startDatH: '+ CONVERT( VARCHAR(30), @startDate , 112)
								...
								SELECT 										[post_tran_id],
										...
										[retrieval_reference_nr] ,
										[recon_business_date],
										,master.dbo.fn_rpt_archive_icc_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  @@SERVERNAME,'H:\postilon_office_xml_data\icc_data_req', REPLACE(CONVERT(VARCHAR(10), t.recon_business_date,111),'/', ''), icc_data_req)  icc_data_req
										 ,master.dbo.fn_rpt_archive_icc_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  @@SERVERNAME,'H:\postilon_office_xml_data\icc_data_rsp',  REPLACE(CONVERT(VARCHAR(10), t.recon_business_date,111),'/', ''), icc_data_req)  icc_data_rsp
										 ,master.dbo.fn_rpt_archive_structured_data_req(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  @@SERVERNAME,'H:\postilon_office_xml_data\structured_data_req',  REPLACE(CONVERT(VARCHAR(10), t.recon_business_date,111),'/', ''), structured_data_req)  structured_data_req
										 ,master.dbo.fn_rpt_archive_structured_data_rsp(post_tran_id ,post_tran_cust_id ,tran_nr , retrieval_reference_nr,  system_trace_audit_nr,  @@SERVERNAME,'H:\postilon_office_xml_data\structured_data_rsp',  REPLACE(CONVERT(VARCHAR(10), t.recon_business_date,111),'/', ''),  structured_data_rsp)   structured_data_rsp
