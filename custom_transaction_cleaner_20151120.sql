...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 11/20/2015 15:49:20 ******/
SET ANSI_NULLS ON
GO
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  
	...
	DECLARE @max_date_time  DATETIME;  
	   ...
	   --SET @datetime_start=DATEADD(MINUTE, @period_interval, @datetime_temp);  
		...
		WHILE (DATEDIFF(MINUTE,@datetime_start, @endDate)>0)   
		 BEGIN  
		  ...
		  SELECT top 1 @last_post_tran_id = post_tran_id FROM  post_tran trans WITH(NOLOCK, INDEX(ix_post_tran_7))  WHERE  datetime_req <=@datetime_stop ORDER BY datetime_req DESC  
		  ...
		  DELETE FROM extract_tran WHERE post_tran_id <=  @last_post_tran_id ;  
			
		  SET ROWCOUNT @batch_size  
		  DELETE_POST_TRAN:  
		  DELETE FROM post_tran  WHERE datetime_req < @datetime_stop;  --post_tran_id <=  @last_post_tran_id 
		  IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN  
		  SET ROWCOUNT 0  
	  
		  SET ROWCOUNT @batch_size  
		  DELETE_POST_TRAN_CUST:  
		  DELETE post_tran_cust FROM post_tran WITH(NOLOCK, INDEX(ix_post_tran_7))  JOIN post_tran_cust ON post_tran.post_tran_cust_id = post_tran_cust.post_tran_cust_id WHERE    datetime_req < @datetime_stop;  
		  --DELETE FROM post_tran_cust  WHERE  post_tran_cust_id <= @last_post_tran_cust_id ;  
		 ...
		 --COMMIT TRANSACTION  
		 	   SET @datetime_start =@datetime_stop
		 END  
		  PRINT 'Running final clean up for transactions older than: '+CHAR(10)  
		  PRINT 'Cleaning post_tran records older than: '+@datetime_stop   
		  SET @datetime_stop = DATEADD(D,0, DATEDIFF(D,0,@datetime_stop))  
		  SET @last_post_tran_id  = (SELECT TOP 1 trans.post_tran_id FROM post_tran trans WITH(NOLOCK, INDEX(ix_post_tran_7))  JOIN post_tran_cust cust ON trans.post_tran_cust_id = cust.post_tran_cust_id WHERE datetime_req < CONVERT(DATETIME, @datetime_stop) ORDER BY datetime_req DESC)  
		  DELETE FROM extract_tran WHERE post_tran_id <=  @last_post_tran_id ;  
		  SET ROWCOUNT @batch_size  
		  ...
		  DELETE post_tran_cust FROM post_tran WITH(NOLOCK) INNER JOIN post_tran_cust ON post_tran.post_tran_cust_id = post_tran_cust.post_tran_cust_id WHERE  datetime_req < CONVERT(DATETIME, @datetime_stop) ;  
