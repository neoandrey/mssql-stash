...
SELECT [post_tran_id]
      ...
      ,[source_node_key]
      ,[proc_online_system_id]
  FROM [postilion_office].[dbo].[post_tran] t (NOLOCK)
      JOIN
...
WHERE 
 ...
 (post_tran_id<=@last_post_tran_id)  AND
...
CREATE NONCLUSTERED INDEX [ix_post_tran_temp_44]
...
FROM  dbo.sstl_journal_all AS J (NOLOCK)JOIN
   (  
    select [Date]as Business_date   from  dbo.get_dates_in_range(@from_date,@to_date)
    
    ) r
    ON J.Business_date   = r. Business_date
...
ON (rdate.recon_business_date = PT.recon_business_date AND rdate.recon_business_date=  J.business_date )
--left join #post_tran_temp ptt (NOLOCK, INDEX(ind_post_tran_temp_2))
