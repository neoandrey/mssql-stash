...
print 'post_online_batch  truncated'
truncate table [post_settle_entity]
print 'post_settle_entity  truncated'
truncate table  post_batch
print 'post_batch  truncated'
...
SELECT  normalization_session_id =  ROW_NUMBER() OVER (ORDER BY (SELECT  1)),  online_system_id = CASE WHEN online_system_id = 1  THEN 2
                               WHEN online_system_id = 2  THEN 1
			  END,datetime_creation, first_post_tran_id = 1, first_post_tran_cust_id=1, completed
FROM (
SELECT * FROM  OPENQUERY([172.25.10.89], 'SELECT top 10 * FROM post_normalization_session a WITH  (NOLOCK) JOIN  post_norm_rtfw_session b WITH  (NOLOCK)   ON a.normalization_session_id = b.session_id WHERE datetime_creation <=''2018-06-23 22:00:00'' order  by session_id  DESC')
)A

insert into post_norm_rtfw_session

SELECT 
  session_id  =  ROW_NUMBER() OVER (ORDER BY (SELECT  1)),
  last_tran_legs_nr,
  last_datetime,
  copied_batches,
  is_dst
