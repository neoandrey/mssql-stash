...
where cust.terminal_id = @terminal_id
AND cust.source_node_name IN ('WEBSWTsrc','WEBsrc')
...
DECLARE @rrn_sttl_impact_total VARCHAR(20);
DECLARE @excluded_rrns TABLE (retrieval_reference_nr VARCHAR(20));

DECLARE rrn_cursor CURSOR LOCAL FORWARD_ONLY STATIC READ_ONLY FOR SELECT DISTINCT retrieval_reference_nr FROM #temp_batch_data_2;

OPEN rrn_cursor;

FETCH NEXT FROM rrn_cursor INTO @retrieval_reference_nr;

WHILE (@@FETCH_STATUS=0) 
BEGIN

	SELECT @rrn_sttl_impact_total = SUM(settle_amount_impact) WHERE retrieval_reference_nr = @retrieval_reference_nr;
	IF(@rrn_sttl_impact_total=0)
	BEGIN
		INSERT INTO @excluded_rrns(retrieval_reference_nr) VALUES (@retrieval_reference_nr);
	END

FETCH NEXT FROM rrn_cursor INTO @retrieval_reference_nr;
END
CLOSE rrn_cursor;
DEALLOCATE rrn_cursor;

SELECT * FROM #temp_batch_data_2 WHERE retrieval_reference_nr NOT IN (SELECT retrieval_reference_nr FROM @excluded_rrns);
