...
/****** Object:  StoredProcedure [dbo].[usp_copy_settlement_transaction_details]    Script Date: 03/26/2017 14:11:40 ******/
SET ANSI_NULLS ON
GO
...
create index ix_post_tran_id on ##settle_tran_details(
ptid
)
...
,pan  pan
,ptc_post_tran_cust_id post_tran_cust_id  
,pt_post_tran_id post_tran_id
into ##final_results_tables
        from (SELECT * from ##TEMP_POST_TRAN_DATA_LOCAL t (NOLOCK)  
        JOIN ##settle_tran_details s (nolock)  ON t.PT_post_tran_id = s.PTID AND  t.pt_message_type IN (''0200'',''0220'')
) pt
...
left JOIN tbl_merchant_category m (NOLOCK)
			 	...
			 	---SELECT * INTO   ##final_results_tables FROM ##settle_tran_details s (nolock) JOIN ##semi_final_results_tables PT (NOLOCK)  on s.ptid = pt.post_tran_id_1	
					...
					DROP TABLE ##settle_tran_details
					END
				INSERT INTO postilion_office.dbo.['+@previous_table_name+'] SELECT DISTINCT * FROM  ##final_results_tables;	
...
left JOIN tbl_merchant_category m (NOLOCK)
			 	...
			 	---SELECT * INTO   ##final_results_tables FROM ##settle_tran_details s (nolock) JOIN ##semi_final_results_tables PT (NOLOCK)  on s.ptid = pt.post_tran_id_1	
					...
					DROP TABLE ##settle_tran_details
					END
				INSERT INTO postilion_office.dbo.['+@table_name+'] SELECT DISTINCT * FROM  ##final_results_tables;	
