...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown]    Script Date: 12/16/2016 17:27:36 ******/
SET ANSI_NULLS ON
GO
...
INSERT            INTO settlement_summary_session VALUES (@settle_business_date)
...
END,
      trxn_category=CASE WHEN (PT.PT_tran_type ='01')  
                          ...
                          or SUBSTRING(PT.PTC_terminal_id,1,1)= '0' or SUBSTRING(PT.PTC_terminal_id,1,1)= '4')) THEN 'CARD HOLDER ACCOUNT TRANSFER'
                           ...
                           and dbo.fn_rpt_isPurchaseTrx_sett(PT.PT_tran_type,PT.PTC_source_node_name, PT.PT_sink_node_name,PT.PTC_terminal_id ,PT.PTC_totals_group ,PT.PTC_pan) = 1) THEN 'WEB(FLAT FEE OF N50)PURCHASE'                     
...
END,
      ...
      trxn_date=j.business_date,
        ...
        Issuer = (case when (not ((DebitAccNr_acc_nr LIKE 'ISW%' and DebitAccNr_acc_nr not LIKE '%POOL%' ) OR (CreditAccNr_acc_nr LIKE 'ISW%' and CreditAccNr_acc_nr not LIKE '%POOL%' ) )) then ''
           ...
           PT.PT_tran_nr ,  --   Volume = count(PT.PT_tran_nr),
              PT.PT_settle_amount_req, --Value_RequestedAmount = sum(PT.PT_settle_amount_req)/100,
            PT.PT_settle_amount_impact,--Value_SettleAmount = sum(PT.PT_settle_amount_impact*-1)/100,
                      
                 index_no = IDENTITY(INT,1,1)
          INTO #report_result_temp FROM
...
WHERE
    ...
    (
      ...
      and NOT (PT.PTC_totals_group  =  ('VISAGroup') and PT.PT_acquiring_inst_id_code = '627787')
        and NOT (PT.PTC_totals_group =  ('VISAGroup') and PT.PT_sink_node_name  <>('ASPPOSVINsnk')
...
(bank_code,trxn_category,Debit_Account_type,Credit_Account_type,trxn_amount,trxn_fee,trxn_date, Currency,late_reversal,card_type,terminal_type, Acquirer, Issuer,Volume,Value_RequestedAmount,Value_SettleAmount)       
       SELECT 
              bank_code,trxn_category,Debit_Account_type,Credit_Account_type,sum(trxn_amount),sum(trxn_fee),trxn_date, Currency,late_reversal,card_type,terminal_type, Acquirer, Issuer,Count(Volume),Sum(Value_RequestedAmount),Sum(Value_SettleAmount) 
       FROM 
                     #report_result 
where     index_no not IN (SELECT index_no FROM  #report_result where source_node_name in ('SWTNCS2src','SWTSHOPRTsrc','SWTNCSKIMsrc','SWTNCSKI2src','SWTFBPsrc','SWTUBAsrc','SWTZIBsrc','SWTPLATsrc') and unique_key  IN (SELECT unique_key FROM #temp_table))
