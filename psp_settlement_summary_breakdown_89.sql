...
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown]    Script Date: 7/12/2017 3:18:36 PM ******/
SET ANSI_NULLS ON
GO
...
WHEN                      
  ...
  WHEN                      (DebitAccNr_acc_nr LIKE '%ISSUER_FEE_PAYABLE' OR CreditAccNr_acc_nr LIKE '%ISSUER_FEE_PAYABLE'
                          ...
                          and (dbo.fn_rpt_isBillpayment (PT.PTC_terminal_id,PT.PT_extended_tran_type,PT.PT_message_type,PT.PT_sink_node_name,PT.PT_payee,PT.PTC_card_acceptor_id_code ,PT.PTC_source_node_name,PT.PT_tran_type,PT.PTC_pan) = 0)
                          AND dbo.fn_rpt_CardGroup(PT.PTC_pan) = '6'
                        AND  PT.PT_acquiring_inst_id_code not in ('639139','627787') 
...
(bank_code,trxn_category,Debit_Account_type,Credit_Account_type,trxn_amount,trxn_fee,trxn_date, Currency,late_reversal,card_type,terminal_type, Acquirer, Issuer,Volume,Value_RequestedAmount,Value_SettleAmount)       
       SELECT 
              bank_code,trxn_category,Debit_Account_type,Credit_Account_type,sum(trxn_amount),sum(trxn_fee),trxn_date, Currency,late_reversal,card_type,terminal_type, Acquirer, Issuer,Sum(Volume),(Sum(Value_RequestedAmount))/100,(Sum(Value_SettleAmount*-1))/100 
       FROM 
                     #report_result 
