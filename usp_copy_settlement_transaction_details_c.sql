ALTER PROCEDURE usp_copy_settlement_transaction_details  AS
							BEGIN
							...
							DECLARE @settlement_date NVARCHAR(10)
							DECLARE @create_table_sql NVARCHAR(MAX)
							DECLARE @table_name NVARCHAR(MAX)
							SET @settlement_date = CONVERT(VARCHAR(10), GETDATE(),112)
							...
							IF (@start_post_tran_id = 0 OR @start_post_tran_id IS NULL)
							 BEGIN
								...
								SELECT   @end_post_tran_id    = MIN(last_post_tran_id) FROM sstl_session (NOLOCK) where CONVERT(VARCHAR(10), datetime_started,112)  =CONVERT(VARCHAR(10), GETDATE(),112) AND completed = 1;
...
;WITH temp_journal_data_local  AS 
  (  SELECT  
	...
	FROM
	dbo.[sstl_journal_temp] AS J (NOLOCK)
	...
	WHERE post_tran_id  BETWEEN '+@start_post_tran_id+'  AND '+@end_post_tran_id+'	
	...
	FROM   post_tran AS PT  WITH (NOLOCK, INDEX(ix_post_tran_1))
	 LEFT JOIN  Post_tran_cust AS PTC WITH  (NOLOCK, INDEX(pk_post_tran_cust))
	ON (PT.post_tran_cust_id = PTC.post_tran_cust_id and post_tran_id  BETWEEN '+@start_post_tran_id+'  AND '+@end_post_tran_id+'
...
SELECT report_results.*,pt.[PT_post_tran_id] as post_tran_id_1
...
FROM 
(SELECT bank_code,trxn_category,Debit_account_type,Credit_account_type,trxn_amount,trxn_fee,trxn_date,currency,late_reversal,card_type,terminal_type,source_node_name,Unique_key,Acquirer,Issuer,Volume,Value_RequestedAmount,Value_SettleAmount,ptid,ptcid,index_no FROM ##settle_tran_details WHERE index_no not IN (SELECT index_no FROM ##settle_tran_details (NOLOCK) where source_node_name in (''SWTNCS2src'',''SWTSHOPRTsrc'',''SWTNCSKIMsrc'',''SWTNCSKI2src'',''SWTFBPsrc'',''SWTUBAsrc'',''SWTZIBsrc'',''SWTPLATsrc'') and unique_key IN (SELECT unique_key FROM #temp_table))
 )report_results (nolock)
INNER JOIN temp_post_tran_data PT 
...
DROP TABLE   ##settle_tran_details');
							...
							ELSE
								BEGIN
									SELECT   TOP 1 @end_post_tran_id    = (last_post_tran_id) FROM sstl_session (NOLOCK) where CONVERT(VARCHAR(10), datetime_started,112)  =CONVERT(VARCHAR(10), GETDATE(),112) AND completed = 1 AND last_post_tran_id > @start_post_tran_id order by session_id;
									IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'report_result') AND type in (N'U')) BEGIN
...
left JOIN tbl_merchant_category m (NOLOCK)
                        ...
                        left JOIN Reward_Category r (NOLOCK)
                           ...
                           left JOIN tbl_merchant_category_web mer (NOLOCK)
                                                ON PT.PTC_merchant_type = mer.category_code 
                                             LEFT JOIN tbl_terminal_owner ow ON PT.PTC_terminal_id = ow.terminal_id
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''##settle_tran_details'') AND type in (N''U''))
DROP TABLE   ##settle_tran_details');
								END
