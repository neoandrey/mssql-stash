...
DECLARE logfile_cursor CURSOR STATIC READ_ONLY LOCAL FORWARD_ONLY FOR SELECT name FROM postilion_office.sys.master_files WHERE database_id = db_id() AND type = 1;
OPEN logfile_cursor
FETCH NEXT FROM logfile_cursor INTO @current_logfile;
WHILE (@@FETCH_STATUS =0)BEGIN
PRINT('DBCC SHRINKFILE ('+ @current_logfile+',0);')
FETCH NEXT FROM logfile_cursor INTO @current_logfile;
END
