...
CREATE PROCEDURE dbo.pim_routes_duplicate_totals_group (  
...
AS 
	BEGIN
		...
		DECLARE	@draft_capture INT;
		DECLARE	@node VARCHAR(12);
		DECLARE	@currency_code [char](3);
		DECLARE	@tran_group [varchar](30);
	   ...
	   EXEC('CREATE TABLE '+@report_table_name+' (
		...
		EXEC('INSERT INTO '+@report_table_name+'
			 ...
			 SELECT [totals_group]
			   ...
			   ,[tran_group]
			   ,'+@current_user+
			   ','+@current_datetime+' FROM '+@report_table_name);
			   ...
			   SELECT * FROM #temp_tm_route
			   SELECT * FROM #temp_new_tm_routes;
			   SELECT * FROM tm_routes;
