...
SELECT job_id FROM msdb.dbo.sysjobsteps WHERE step_name IN(  SELECT process_entity  FROM postilion_office.dbo.post_process_run WHERE process_name
 LIKE 'Reports' AND result_value =0 AND LEFT(CONVERT(VARCHAR(10),datetime_begin, 112),10)>='20141025' AND 
process_entity NOT IN (SELECT process_entity FROM post_pROcess_run WHERE process_name
 LIKE 'Reports' AND result_value =10 AND LEFT(CONVERT(VARCHAR(10),datetime_begin, 112),10)>='20141025'))AND last_run_date >='20141025'
--INSERT INTO @job_id_table (job_id)
