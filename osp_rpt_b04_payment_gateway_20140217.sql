...
GO
/****** Object:  StoredProcedure [dbo].[osp_rpt_b04_payment_gateway]    Script Date: 02/17/2014 10:14:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE                                                                 PROCEDURE [dbo].[osp_rpt_b04_payment_gateway_2]
...
BEGIN
	...
	INSERT INTO  @list_of_BINs SELECT part as 'BIN' FROM usf_split_string(@BINs,',') EXEC osp_rpt_util_split_nodenames @BINs
	...
	WHERE 		
			...
			c.terminal_id like '3CPD%' and tran_type = ('00')
			OR
			...
			OR LEFT(pan,6) = '628051' and (SUBSTRING(c.totals_group,1,3)in (SELECT bank_code FROM @list_of_bank_codes)) and source_node_name NOT IN (SELECT source_node FROM @list_of_source_nodes)
			) 
            AND
