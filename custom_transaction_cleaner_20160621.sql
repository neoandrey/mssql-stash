...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner_20160620]    Script Date: 06/21/2016 11:39:14 ******/
SET ANSI_NULLS ON
GO
   ...
   INSERT INTO @recon_business_date_table  SELECT [DATE] FROM dbo.get_dates_in_range(@datetime_start, @last_day_to_clean);  
    ...
    WHILE (DATEDIFF(day,@current_clean_date, @last_day_to_clean)>=0) BEGIN  
      ...
      BEGIN TRY  
        ...
        DELETE_POST_TRAN:  
        DELETE FROM dbo.post_tran   WHERE   post_tran_id in (SELECT  TOP (@batch_size)  post_tran_id FROM post_tran (NOLOCK, index(ix_post_tran_9)) WHERE recon_business_date >= (SELECT MIN(RECON_BUSINESS_DATE)  FROM post_tran ( NOLOCK, INDEX(ix_post_tran_9))) AND  recon_business_date <=@current_clean_date)  ;  
        ...
        DELETE FROM dbo.post_tran_cust   WHERE   post_tran_cust_id BETWEEN @first_post_tran_cust_id  AND @last_post_tran_cust_id   ;  
