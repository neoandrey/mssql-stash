...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 07/22/2016 08:33:24 ******/
SET ANSI_NULLS ON
GO
   ...
   INSERT INTO @recon_business_date_table  SELECT [DATE] FROM dbo.get_dates_in_range(@datetime_start, @last_day_to_clean);  
    ...
    WHILE (DATEDIFF(day,@current_clean_date, @last_day_to_clean)>=0) BEGIN  
      ...
      BEGIN TRY  
        ...
        DELETE_EXTRACT_TRAN:  
        DELETE  extract_tran FROM extract_tran ext JOIN 

   (  SELECT  TOP (@batch_size) post_tran_id   FROM post_tran t3  WITH  (NOLOCK, INDEX(ix_post_tran_7)) 
          JOIN
          ( SELECT [DATE] recon_business_date FROM dbo.get_dates_in_range(@current_clean_date, @current_clean_date))cld 
        ON 
		cld.recon_business_date = t3.recon_business_date ORDER BY DATETIME_REQ) t2
ON ext.post_tran_id = t2.post_tran_id
        ...
        SET ROWCOUNT @batch_size  
        DELETE_POST_TRAN:  
        DELETE  dbo.post_tran  FROM
				  post_tran  t1 WITH  (NOLOCK, INDEX(ix_post_tran_1))
				  JOIN (  SELECT  TOP  (@batch_size) post_tran_id   FROM post_tran t3  WITH  (NOLOCK, INDEX(ix_post_tran_7)) 
						  JOIN
						  ( SELECT [DATE] recon_business_date FROM dbo.get_dates_in_range(@current_clean_date, @current_clean_date))cld 
						ON 
						cld.recon_business_date = t3.recon_business_date ORDER BY DATETIME_REQ) t2
				ON t1.post_tran_id = t2.post_tran_id
