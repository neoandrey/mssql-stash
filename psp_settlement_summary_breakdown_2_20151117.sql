...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown_2]    Script Date: 11/17/2015 18:21:28 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO

ALTER                                                                                                                     PROCEDURE [dbo].[psp_settlement_summary_breakdown_2](
...
INSERT 
       ...
       FROM   dbo.sstl_journal_all AS J (NOLOCK), post_tran AS PT (NOLOCK),post_tran_cust as PTC (NOLOCK)
        ...
        where PT.rsp_code_rsp in ('00','11','09')
              ...
              or (PT.settle_amount_rsp<> 0 and PT.message_type = '0420' and PT.tran_reversed <> 2 and PT.tran_type = 40 and (SUBSTRING(PTC.Terminal_id,1,1)= '1' or SUBSTRING(PTC.Terminal_id,1,1)= '0')) )
              AND ( PT.post_tran_id >=@first_post_tran_id AND PT.post_tran_id<=@last_post_tran_id AND J.business_date >= @from_date )
...
WHEN                     (CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0
                          OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0  OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0 
                          OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0  OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )
...
WHEN                                      ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )
                          and dbo.fn_rpt_isPurchaseTrx_sett(PT.tran_type, PTC.source_node_name, PT.sink_node_name, PTC.terminal_id ,ptc.totals_group ,ptc.pan ) = 1
                          and (dbo.fn_rpt_isBillpayment (PTC.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,ptc.card_acceptor_id_code ,ptc.source_node_name,pt.tran_type,ptc.pan) = 0)
                          AND dbo.fn_rpt_CardGroup(ptc.pan) = '6'
 ...
 WHEN             
                     ...
                     (CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0       
                          OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0  OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )
                           and (dbo.fn_rpt_isBillpayment (PTC.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,ptc.card_acceptor_id_code ,ptc.source_node_name,pt.tran_type,ptc.pan) = 1)
                          AND dbo.fn_rpt_CardGroup(ptc.pan) = '6'
                          --AND PT.acquiring_inst_id_code <> '627480' 
...
WHEN PTT.Retention_data = '1046' and 
                               ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) THEN 'UBN'
WHEN PTT.Retention_data ='9130' and 
                                  ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'ABS'
WHEN PTT.Retention_data ='9044' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )  then 'ABP'
WHEN PTT.Retention_data ='9023'  and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'CITI'
WHEN PTT.Retention_data ='9050' and 
                                      ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'EBN'
WHEN PTT.Retention_data ='9214' and 
                                      ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'FCMB'
WHEN PTT.Retention_data ='9070' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )  ) then 'FBP'
WHEN PTT.Retention_data ='9011' and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )  then 'FBN'
WHEN PTT.Retention_data ='9058'  and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'GTB'
WHEN PTT.Retention_data ='9082' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'KSB'
WHEN PTT.Retention_data ='9076' and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )  ) then 'SKYE'
WHEN PTT.Retention_data ='9084' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'ENT'
WHEN PTT.Retention_data ='9039' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )  then 'IBTC'
WHEN PTT.Retention_data ='9068' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )  then 'SCB'
WHEN PTT.Retention_data ='9232' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )  )  then 'SBP'
...
WHEN PTT.Retention_data ='9033'  and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'UBA'
WHEN PTT.Retention_data ='9215'  and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )  ) then 'UBP'
WHEN PTT.Retention_data ='9035' and 
                                        ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) )  then 'WEMA'
WHEN PTT.Retention_data ='9057' and 
                                   ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 ) ) then 'ZIB'
WHEN PTT.Retention_data ='9301' and 
                                       ((CHARINDEX('ISSUER_FEE_PAYABLE', DebitAccNr.acc_nr) >0 OR  CHARINDEX('ISSUER_FEE_RECEIVABLE', DebitAccNr.acc_nr) >0 OR CHARINDEX('AMOUNT_PAYABLE', DebitAccNr.acc_nr) >0) 
 OR 
(CHARINDEX('ISSUER_FEE_PAYABLE', CreditAccNr.acc_nr) >0 OR CHARINDEX('ISSUER_FEE_RECEIVABLE', CreditAccNr.acc_nr) >0   OR  CHARINDEX('AMOUNT_PAYABLE', CreditAccNr.acc_nr) >0 )  )  then 'JBP'
...
END,
	trxn_category=CASE WHEN (PT.tran_type ='01'  AND (LEFT(PTC.Terminal_id,1) IN ('1', '0'))) 
                           ...
                           WHEN (PT.tran_type ='01'  AND (LEFT(PTC.Terminal_id,1) IN ('1', '0'))) 
			   ...
			   WHEN (dbo.fn_rpt_isBillpayment (PTC.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,ptc.card_acceptor_id_code ,ptc.source_node_name,pt.tran_type,ptc.pan) = '1') then 'BILLPAYMENT'
                           ...
                           WHEN (PT.tran_type ='40'  AND LEFT(PTC.Terminal_id,1) IN ('1', '0','4')) THEN 'CARD HOLDER ACCOUNT TRANSFER'
...
END,
	...
	business_date=j.business_date,
        ...
        Acquirer = (case when (not ((DebitAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) )) then ''
                      when ((DebitAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) ) AND (acc.acquirer_inst_id1 = PT.acquiring_inst_id_code or acc.acquirer_inst_id2 = PT.acquiring_inst_id_code or acc.acquirer_inst_id3 = PT.acquiring_inst_id_code or acc.acquirer_inst_id4 = PT.acquiring_inst_id_code or acc.acquirer_inst_id5 = PT.acquiring_inst_id_code) then acc.bank_code --or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code1 or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code2 or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code3 or SUBSTRING(PTC.terminal_id,2,3) = acc.cbn_code4) THEN acc.bank_code
                      else PT.acquiring_inst_id_code END),
        Issuer = (case when (not ((DebitAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) )) then ''
                      when ((DebitAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) ) and (substring(ptc.totals_group,1,3) = acc.bank_code1) then acc.bank_code
...
(case when (not ((DebitAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', CreditAccNr.acc_nr)<1  ) )) then ''
                      when ((DebitAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) ) AND (acc.acquirer_inst_id1 = PT.acquiring_inst_id_code or acc.acquirer_inst_id2 = PT.acquiring_inst_id_code or acc.acquirer_inst_id3 = PT.acquiring_inst_id_code or acc.acquirer_inst_id4 = PT.acquiring_inst_id_code or acc.acquirer_inst_id5 = PT.acquiring_inst_id_code) then acc.bank_code --or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code1 or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code2 or SUBSTRING(PTC.terminal_id,2,3)= acc.cbn_code3 or SUBSTRING(PTC.terminal_id,2,3) = acc.cbn_code4) THEN acc.bank_code
                      else PT.acquiring_inst_id_code END),
(case when (not ((DebitAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and  CHARINDEX('POOL', CreditAccNr.acc_nr)<1 ) )) then ''
                      when ((DebitAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', DebitAccNr.acc_nr)<1 ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and CHARINDEX('POOL', CreditAccNr.acc_nr)<1  ) ) and (substring(ptc.totals_group,1,3) = acc.bank_code1) then acc.bank_code1
                      else substring(ptc.totals_group,1,3) END),
...
(bank_code,trxn_category,Debit_Account_type,Credit_Account_type,trxn_amount,trxn_fee,trxn_date, Currency,late_reversal,card_type,terminal_type, Acquirer, Issuer)	
	...
	FROM 
			#report_result 
where     not(source_node_name in ('SWTNCS2src','SWTSHOPRTsrc','SWTNCSKIMsrc') 
