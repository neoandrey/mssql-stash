...
/****** Object:  StoredProcedure [dbo].[usp_settlement_summary_breakdown_details_check]    Script Date: 04/29/2017 06:55:28 ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[usp_settlement_summary_breakdown_details_check] AS
BEGIN
 ...
 IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[settlement_breakdown_details_check]') AND type in (N'U'))begin
	DROP TABLE [dbo].[settlement_breakdown_details_check]
			 ...
			 INCLUDE (PT_post_tran_id)
			 														...
			 														DELETE FROM [settlement_breakdown_details_check]
			 														  WHERE index_no IN (SELECT index_no FROM [settlement_breakdown_details_check] (NOLOCK) where  (source_node_name 
			 														 IN ('SWTNCS2src','SWTSHOPRTsrc','SWTNCSKIMsrc','SWTNCSKI2src','SWTFBPsrc','SWTUBAsrc'
			 														 ,'SWTZIBsrc','SWTPLATsrc') and PT_post_tran_id IN (select PT_post_tran_id from [settlement_breakdown_details_check]  (NOLOCK) where source_node_name in ('SWTASPPOSsrc','SWTASGTVLsrc')
			 														)))
			 
			 

			 
			 CREATE NONCLUSTERED INDEX [ix_settlement_breakdown_details_check_4]
			 ON [dbo].[settlement_breakdown_details_check] ([index_no])
			 
			 

			
			 CREATE NONCLUSTERED INDEX [ix_settlement_breakdown_details_check_5]
			 ON [dbo].[settlement_breakdown_details_check] ([Unique_key],[source_node_name])
			 INCLUDE ([index_no])	
			 
			 	 		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[settlement_breakdown_details_table]') AND type in (N'U'))begin
						DROP TABLE [dbo].settlement_breakdown_details_table
						end										  
			 SELEct   
			 Acquirer
			 ,PT.bank_code
