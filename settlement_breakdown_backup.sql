...
GO
/****** Object:  StoredProcedure [dbo].[psp_settlement_summary_breakdown]    Script Date: 11/27/2015 6:13:38 PM ******/
SET ANSI_NULLS ON
GO
...
END,
	trxn_category=CASE WHEN (PT.tran_type ='01'  AND (LEFT(PT.Terminal_id,1)= '1' or LEFT(PT.Terminal_id,1)= '0')) 
                           ...
                           WHEN (PT.tran_type ='01'  AND (LEFT(PT.Terminal_id,1)= '1' or LEFT(PT.Terminal_id,1)= '0')) 
			   ...
			   WHEN (dbo.fn_rpt_isBillpayment (PT.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,PT.card_acceptor_id_code ,PT.source_node_name,pt.tran_type,PT.pan) = '1') then 'BILLPAYMENT'
                           ...
                           WHEN (PT.tran_type ='40'  AND (LEFT(PT.Terminal_id,1)= '1' 

                           or LEFT(PT.Terminal_id,1)= '0' or LEFT(PT.Terminal_id,1)= '4')) THEN 'CARD HOLDER ACCOUNT TRANSFER'
...
WHERE 
     ...
     and PT.recon_business_date <= @to_date 
      ...
      AND
             PT.source_node_name  NOT LIKE 'SB%'
             AND
             pt.sink_node_name  NOT LIKE 'SB%'

      and not(PT.source_node_name  LIKE '%TPP%')
       and not(pt.sink_node_name  LIKE '%TPP%')
       and not (PT.source_node_name  = 'MEGATPPsrc' and pt.tran_type = '00')
      --and source_node_name not in ('SWTWMASBsrc','SWTWEMSBsrc')
      and PT.source_node_name NOT LIKE 'SWTMEGADSsrc'
