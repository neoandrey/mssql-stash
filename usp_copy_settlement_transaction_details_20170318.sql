...
GO
/****** Object:  StoredProcedure [dbo].[usp_copy_settlement_transaction_details]    Script Date: 03/18/2017 18:02:08 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[usp_copy_settlement_transaction_details] (@settlement_date NVARCHAR(10), @retention_period INT)  AS
							...
							END
														...
														,[proc_online_system_id] FROM   post_tran AS PT1 WITH (NOLOCK, INDEX(ix_post_tran_9))
														   WHERE   post_tran_id  > '+@start_post_tran_id+'  AND post_tran_id <='+@end_post_tran_id+' AND  recon_business_date = '''+@previous_settlement_date+''') PT
...
EXEC('
								...
								,source_node_key
								,[proc_online_system_id] FROM   post_tran AS PT1 WITH (NOLOCK, INDEX(ix_post_tran_9))
								   WHERE post_tran_id   > '+@start_post_tran_id+' and post_tran_id  <='+@end_post_tran_id+'  AND  recon_business_date = '''+@settlement_date+''') PT
									 LEFT JOIN  Post_tran_cust AS PTC WITH  (NOLOCK, INDEX(pk_post_tran_cust))
									ON (PT.post_tran_cust_id = PTC.post_tran_cust_id 
									 AND  LEFT( source_node_name,2 ) <> ''SB''
									AND  CHARINDEX(''TPP'',source_node_name )<1
