...
UNION ALL 
select 'BillPaymentLog' TableName,  (SELECT COUNT(TransactionId) from BillPaymentLog with (nolock, INDEX(pk_BillPaymentLog_Part))) 'Original', (SELECT COUNT(TransactionId) from BillPaymentLog_Part (nolock)) 'Partitioned'
UNION ALL 
...
[ThirdPartyData],[STAN],[Bin],[IsSvaTransaction]  
 ...
 FROM
  ...
  WHERE ID  not in (
  SELECT ID FROM transactions_part  WITH (NOLOCK, INDEX(PK_Bills_PushContents_part ))
  )
