			       ...
			       GO
			       /****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 09/01/2015 15:39:21 ******/
			       SET ANSI_NULLS ON
			       GO
			       			...
			       			END
			       				...
			       				WHILE (DATEDIFF(MINUTE,@datetime_start, @datetime_stop)>0) 
			       					BEGIN
			       						...
			       						DELETE FROM extract_tran WHERE post_tran_id <=  @last_post_tran_id OPTION(MAXDOP  16);
			       						
			       						SET ROWCOUNT 4000
			       						DELETE_POST_TRAN:
			       						DELETE FROM post_tran  WHERE  datetime_req  <= @datetime_start OPTION(MAXDOP 16);
			       						IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN
			       						SET ROWCOUNT 0
			       
			       						SET ROWCOUNT 4000
			       						DELETE_POST_TRAN_CUST:
			       						DELETE post_tran_cust FROM post_tran WITH(NOLOCK) INNER JOIN post_tran_cust ON post_tran.post_tran_cust_id = post_tran_cust.post_tran_cust_id WHERE datetime_req < CONVERT(DATETIME, @datetime_start);
			       						...
			       						DBCC SHRINKFILE (post_office_log_extd,0);
			       DBCC SHRINKFILE (postilion_office_2_Log,0);
			       DBCC SHRINKFILE (postilion_office_3_Log,0);
			       DBCC SHRINKFILE (postilion_office_4_Log,0);
			       DBCC SHRINKFILE (postilion_office_5_log,0);
