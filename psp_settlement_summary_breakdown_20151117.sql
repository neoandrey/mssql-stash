...
CREATE                                                                                                                     PROCEDURE [dbo].[psp_settlement_summary_breakdown_2](
...
INSERT 
       ...
       FROM   dbo.sstl_journal_all AS J (NOLOCK), post_tran AS PT (NOLOCK, INDEX(ix_post_tran_7)),post_tran_cust as PTC (NOLOCK)
        ...
        where PT.rsp_code_rsp in ('00','11','09')
              ...
              AND (J.business_date >= @from_date AND PT.post_tran_id >=@first_post_tran_id AND PT.post_tran_id<=@last_post_tran_id )
...
RIGHT OUTER JOIN dbo.post_tran AS PT (NOLOCK , INDEX(ix_post_tran_7))
...
left join post_tran ptt (nolock , INDEX(ix_post_tran_7)) 
...
WHERE 
      ...
      AND
     PT.recon_business_date >= @from_date   AND PTC.post_tran_cust_id >= @rpt_tran_id 
