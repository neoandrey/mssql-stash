...
GO
/****** Object:  StoredProcedure [dbo].[osp_rpt_b04_payment_gateway_2]    Script Date: 02/20/2014 11:38:11 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	WHILE (@@FETCH_STATUS=0)
	  BEGIN
		...
		SET @sink_node_name_new  =  substring(@sink_node_name,1, 3) 	  
	...
	FROM
			...
			post_tran_cust c (NOLOCK) ON (t.post_tran_cust_id = c.post_tran_cust_id)
			 AND 
			(CONVERT(BIGINT,c.post_tran_cust_id) >= CONVERT(BIGINT,@rpt_tran_id))
	WHERE 		
			t.tran_completed = 1
			AND
			...
			AND
              ...
              CHARINDEX( c.terminal_id, '3EPY0701 3IPD0010 3IPDTROT 3VRV0001 3IGW0010 3SFX0014') > 0
                        OR 
		    (c.terminal_id = '3BOL0001' AND t.extended_tran_type = '8502')
		     OR
		    CHARINDEX( LEFT(c.terminal_id ,4),  '3IGW 3CCW 3IBH 3011 3CPD 3SFA 3CPD')>0 OR 	LEFT(c.terminal_id ,5) = '3ADPS'
           	)
			)
			OR
			(CHARINDEX('3CPD', c.terminal_id ) > 0 and tran_type = ('00'))
			OR
			  CHARINDEX( '3IPDFDT', c.terminal_id ) > 0 OR  CHARINDEX( '3QTL002', c.terminal_id ) > 0 AND CHARINDEX(message_type, '0200 0420' ) > 0 AND source_node_name <>'VTUsrc'
			 
			)          
         		
			AND 
			( 
			   (@SinkNode IS NULL OR LEN(@SinkNode) = 0) OR (t.sink_node_name IN (SELECT sink_node FROM @list_of_sink_nodes)) 
			OR (SUBSTRING(t.sink_node_name,4,3) IN (SELECT bank_code FROM @list_of_bank_codes))--and source_node_name NOT IN (SELECT source_node FROM @list_of_source_nodes))
			OR (LEFT(pan,6)IN (SELECT BIN FROM @list_of_BINs)AND source_node_name NOT IN (SELECT source_node FROM @list_of_source_nodes))
			OR CHARINDEX('628051', pan ) > 0 and (LEFT(c.totals_group,3)in (SELECT bank_code FROM @list_of_bank_codes)) and source_node_name NOT IN (SELECT source_node FROM @list_of_source_nodes)
