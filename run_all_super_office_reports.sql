...
FROM         (SELECT DISTINCT md.md_media_iid, cn.ATTRIBUTE_VALUE, md.MD_BUCKET_IID_HOST, tt.TC_TXN_IID, md.account_id,
                                                  (SELECT    MD_UMI
...
(select COALESCE(new_value,'0') from xls_admin.tc_txn_attribute TTATTR where tc_txN_iid = tt.tc_txn_iid and ttattr.attribute_iid in (101)AND new_value !='NULL') TBF_AMOUNT10  
 ...
 MD.md_type IN ('EID', 'XM') 
                       ...
                       Tx_hist  WHERE TTA.ENTITY_IID IN  (42,162)
                       AND 
                       TTA.TXN_TYPE IN (1,2,3,5)
GROUP BY 
Tx_hist.tc_txn_iid, Tx_hist.attribute_value, Tx_hist.account_id, Tx_hist.account_details, Tx_hist.txn_type, Tx_hist.txn_date, Tx_hist.txn_ext_ref, Tx_hist.entity_iid, 
