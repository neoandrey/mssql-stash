SET NOCOUNT ON; +
									 ...
									 online_system_id ,participant_id  INTO #TEMP_RECON_OFFICE_+tableNamesuffix+ 
						FROM ( SELECT  * FROM    post_tran  tt(NOLOCK, INDEX(ix_post_tran_9))  
						JOIN (SELECT date [rec_bus_date] 
						FROM dbo.get_dates_in_range('+startDatetimePicker.Value.Date.ToString()+','+endDatetimePicker.Value.Date.ToString()+') ) r 
						ON tt.recon_business_date  =r.rec_bus_date ) t 
						JOIN post_tran_cust c ON t.post_tran_cust_id = c.post_tran_cust_id
						AND  t.tran_completed = 1	AND	t.tran_postilion_originated = 0	AND	(tran_type NOT IN ('31','50','21') 
						AND LEFT( source_node_name,2)  != 'SB'  AND LEFT( sink_node_name,2)  != 'SB' 
						AND c.merchant_type not in ('5371','2501','2504','2505','2506','2507','2508','2509','2510','2511')) t+
                       JOIN recon_client_data_settle_unmatched_+tableNamesuffix+ rec (NOLOCK)    ;
                    StringBuilder subQry = new StringBuilder();