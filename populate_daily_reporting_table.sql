ALTER PROCEDURE populate_daily_reporting_table 		 @report_date_start DATETIME , @report_date_end DATETIME, @override  INT AS
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	EXEC   postilion_office.dbo.check_norm_cutover_status  @is_cutover_successful= @norm_cutover OUTPUT;
			...
			SET @report_date_end = ISNULL(@report_date_start,REPLACE(CONVERT(VARCHAR(10), DATEADD(D, 0,GETDATE()),111),'/', ''))
			SET @report_date=  @report_date_start 
			SET @override =ISNULL (@override,0);
			SELECT @finished =  ISNULL(finished,0),@index_no=ISNULL(index_no,0) FROM post_tran_report_data_session (nolock) WHERE report_date = @report_date_start;
			...
			SET ROWCOUNT 3000
			DELETE_POST_TRAN_REPORT_DATA:
			DELETE FROM post_tran_report_data  OPTION(MAXDOP 12);
			IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN_REPORT_DATA
			SET ROWCOUNT 0
--REBUILD ALL INDEXES
...
--INSERT DATA
			...
			FROM post_tran trans(NOLOCK)
			  ...
			  WHERE datetime_req>=@report_date_start
			  AND post_tran_id >=@first_post_tran_id AND post_tran_id<= @last_post_tran_id
			  OPTION (MAXDOP 16)
			
			SET @tran_count = @@ROWCOUNT
			IF(@index_no=0) BEGIN
				INSERT INTO post_tran_report_data_session (report_date,start_date, tran_count, finished) VALUES(@report_date,@start_time,@tran_count, @finished );
			END 
			ELSE BEGIN
				UPDATE post_tran_report_data_session SET start_date=@report_date_start,tran_count =@tran_count, finished = @finished WHERE index_no= @index_no;
			END
			
			END
			ELSE BEGIN
				PRINT 'Transactions have already been copied for '+@report_date_start+'. No action performed.';
			END
			END
			ELSE BEGIN
				RAISERROR ('Normalization has not cutover yet. Please consult your administrator for assistance.', 16,  1 );  
				RETURN;
			END
