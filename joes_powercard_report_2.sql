...
SELECT @successful= COUNT(post_tran_cust_id)  FROM post_tran (NOLOCK) WHERE sink_node_name	IN (SELECT sink_node_name FROM #sink_nodes) AND datetime_req  BETWEEN @startDate AND @endDate AND rsp_code_rsp='00';
SELECT @unsuccessful =COUNT(post_tran_cust_id)  FROM post_tran (NOLOCK) WHERE sink_node_name IN (SELECT sink_node_name FROM #sink_nodes) AND datetime_req  BETWEEN @startDate AND @endDate AND rsp_code_rsp='91';
