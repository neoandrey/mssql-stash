USE [post_office_tran_repository]
GO
/****** Object:  StoredProcedure [dbo].[norm_post_tran_mega_summary]    Script Date: 01/18/2017 14:41:59 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE procedure [dbo].[norm_post_tran_mega_summary]
			@date	char(8)
as
BEGIN
	...
	if not exists (select top 1 name from sys.objects where name like 'post_tran_mega_summary_'+@date  and type = 'U' )
	begin
		print 'creating new post_tran_mega_summary partition'
		
		declare @sql_statement varchar(8000)
		set @sql_statement = '
		
		declare @completed_table varchar (50)
		set @completed_table = (select top 1 name from sys.objects  where  LEFT(name,18)= ''post_tran_mega_summary_'' AND ISDATE(RIGHT(name, 8)) =1 and type = ''U'' order by name desc)
		...
		begin
			...
			print ''updating view.....''
			set @view_created = 1
			if not exists (select top 1 * from sys.objects where name = ''post_tran_mega_summary'' and type =''V'')
			begin
				set @action = ''CREATE''
			end
			else
			begin
				...
				exec(@action+'' VIEW dbo.post_tran_mega_summary
				as
		...
		print ''creating table post_tran_mega_summary_'+@date+'''
		CREATE TABLE dbo.post_tran_mega_summary_'+@date+'
		(
		...
		if(@view_created = 0)
		begin
			print ''setting view to new post_tran_mega_summary partition''
			...
			begin
				...
				exec(@action_2+'' VIEW dbo.post_tran_mega_summary
				AS
					SELECT  a.[post_tran_id]
						  ,a.[post_tran_cust_id]
						  ,a.[prev_post_tran_id]
						  ...
						  ,a.[totals_group]
						  ,a.[pan_encrypted]
					  FROM post_tran_mega_summary_'+@date+' a (NOLOCK)'')
		end'
		...
		exec('		
		if not exists (select top 1 * from sys.objects where name = ''post_tran_mega_summary_shadow'' and type =''V'')
		begin
			exec(''CREATE VIEW dbo.post_tran_mega_summary_shadow
			AS
				select  * from post_tran_mega_summary_'+@date+''')
		end
		else
		begin
			exec(''ALTER VIEW dbo.post_tran_mega_summary_shadow
			as
				select  * from post_tran_mega_summary_'+@date+''')
		end
		')
	...
	if(@date_specified = 1)
	begin
		set @current_table = 'post_tran_mega_summary_'+@date
	end
	else
	begin
		set @current_table = (select top 1 name from sys.objects where  LEFT(name,18)= 'post_tran_mega_summary_' AND ISDATE(RIGHT(name, 8)) =1 and type = 'U' order by name desc)
	end
		
	set @table_date = SUBSTRING(@current_table,19,8)
	
	declare @last_post_tran_id bigint
	if exists (select top 1 post_tran_id from post_tran_mega_summary_shadow (nolock))
	begin
		select @last_post_tran_id = (select MAX(post_tran_id) from post_tran_mega_summary_shadow (nolock))
	end
	else
	begin
	...
	set @closed_norm_session_post_tran_id = (select top 1 first_post_tran_id from [172.19.75.71].[postilion_office].post_normalization_session (nolock) where completed =1 order by normalization_session_id desc)
	
	print 'copying data ' ---+ cast(@last_post_tran_id as varchar(16))
	EXEC('INSERT INTO post_tran_mega_summary_shadow  
				SELECT post_tran_id ,a.post_tran_cust_id ,prev_post_tran_id,sink_node_name,tran_postilion_originated,tran_completed,message_type,tran_type,tran_nr ,system_trace_audit_nr,rsp_code_req,rsp_code_rsp,abort_rsp_code,auth_id_rsp,retention_data,acquiring_inst_id_code,message_reason_code,retrieval_reference_nr,datetime_tran_gmt,datetime_tran_local ,datetime_req ,datetime_rsp,realtime_business_date ,recon_business_date ,from_account_type,to_account_type,from_account_id,to_account_id,tran_amount_req,tran_amount_rsp,settle_amount_impact,tran_cash_req,tran_cash_rsp,tran_currency_code,tran_tran_fee_req,tran_tran_fee_rsp,tran_tran_fee_currency_code,settle_amount_req,settle_amount_rsp,settle_tran_fee_req,settle_tran_fee_rsp,settle_currency_code,tran_reversed,prev_tran_approved,extended_tran_type,payee,online_system_id,receiving_inst_id_code,routing_type,source_node_name ,pan,card_seq_nr,expiry_date,terminal_id,terminal_owner,card_acceptor_id_code,merchant_type,card_acceptor_name_loc,address_verification_data,totals_group,pan_encrypted
						FROM	
			OPENQUERY([172.19.75.71],''SELECT post_tran_id ,post_tran.post_tran_cust_id ,prev_post_tran_id,sink_node_name,tran_postilion_originated,tran_completed,message_type,tran_type,tran_nr ,system_trace_audit_nr,rsp_code_req,rsp_code_rsp,abort_rsp_code,auth_id_rsp,retention_data,acquiring_inst_id_code,message_reason_code,retrieval_reference_nr,datetime_tran_gmt,datetime_tran_local ,datetime_req ,datetime_rsp,realtime_business_date ,recon_business_date ,from_account_type,to_account_type,from_account_id,to_account_id,tran_amount_req,tran_amount_rsp,settle_amount_impact,tran_cash_req,tran_cash_rsp,tran_currency_code,tran_tran_fee_req,tran_tran_fee_rsp,tran_tran_fee_currency_code,settle_amount_req,settle_amount_rsp,settle_tran_fee_req,settle_tran_fee_rsp,settle_currency_code,--structured_data_req,tran_reversed,prev_tran_approved,extended_tran_type,payee,online_system_id,receiving_inst_id_code,routing_type,source_node_name ,pan,card_seq_nr,expiry_date,terminal_id,terminal_owner,card_acceptor_id_code,merchant_type,card_acceptor_name_loc,address_verification_data,totals_group,pan_encrypted			
			   FROM (SELECT post_tran_id ,post_tran.post_tran_cust_id ,prev_post_tran_id,sink_node_name,tran_postilion_originated,tran_completed,message_type,tran_type,tran_nr ,system_trace_audit_nr,rsp_code_req,rsp_code_rsp,abort_rsp_code,auth_id_rsp,retention_data,acquiring_inst_id_code,message_reason_code,retrieval_reference_nr,datetime_tran_gmt,datetime_tran_local ,datetime_req ,datetime_rsp,realtime_business_date ,recon_business_date ,from_account_type,to_account_type,from_account_id,to_account_id,tran_amount_req,tran_amount_rsp,settle_amount_impact,tran_cash_req,tran_cash_rsp,tran_currency_code,tran_tran_fee_req,tran_tran_fee_rsp,tran_tran_fee_currency_code,settle_amount_req,settle_amount_rsp,settle_tran_fee_req,settle_tran_fee_rsp,settle_currency_code,--structured_data_req,tran_reversed,prev_tran_approved,extended_tran_type,payee,online_system_id,receiving_inst_id_code,routing_type,FROM post_tran  t WITH (NOLOCK, INDEX(ix_post_tran_9)) 
			 JOIN (SELECT date [rec_business_date] FROM dbo.get_dates_in_range('''''+@table_date+''''','''''+@table_date+''''')) 
			 ON t.recon_business_date = r.rec_business_date 
			) a 
				JOIN  (SELECT post_tran_cust_id,source_node_name ,pan,card_seq_nr,expiry_date,terminal_id,terminal_owner,card_acceptor_id_code,merchant_type,card_acceptor_name_loc,address_verification_data,totals_group,pan_encrypted FROM post_tran_cust (NOLOCK))
	...
	BEGIN
		...
		EXEC   [172.19.75.71].[postilion_office].dbo.check_norm_cutover_status  @is_cutover_successful= @norm_cutover OUTPUT;
		...
		begin
			...
			begin
				...
				set @max_tran_id = (select MAX(post_tran_id) from  [172.19.75.71].[postilion_office].dbo.post_tran where recon_business_date = CONVERT(DATE,@table_date))
				...
				DECLARE   @missing_post_tran_id TABLE  (post_tran_id BIGINT) 
				 ...
				 IF(OBJECT_ID('tempdb.dbo.##remote_mega_post_tran_id') IS NOT NULL) BEGIN
					DROP TABLE ##remote_mega_post_tran_id;
				 END
			     EXEC ('SELECT post_tran_id   INTO ##remote_mega_post_tran_id FROM [172.19.75.71].[postilion_office].[dbo].[post_tran]   WHERE recon_business_date  ='''+@table_date+'''');
				 
				 IF(OBJECT_ID('tempdb.dbo.##local_mega_post_tran_id') IS NOT NULL) BEGIN
					DROP TABLE ##local_mega_post_tran_id;
				 END
			     EXEC ('SELECT post_tran_id   INTO ##local_mega_post_tran_id FROM [postilion_office].[dbo].[post_tran_mega_summary_shadow]   (nolock) WHERE recon_business_date  ='''+@table_date+'''')
					
					INSERT INTO @missing_post_tran_id  select post_tran_id from ##remote_mega_post_tran_id (NOLOCK) WHERE post_tran_id NOT IN (SELECT post_tran_id FROM ##local_mega_post_tran_id)
				  
					INSERT INTO post_tran_mega_summary_shadow
					SELECT post_tran_id ,
				...
				FROM	 ( select post_tran_id ,
					...
					online_system_id,
					receiving_inst_id_code,
					routing_type from [172.19.75.71].[postilion_office].dbo.post_tran aa  (NOLOCK)  
				where  post_tran_id  in (select post_tran_id from @missing_post_tran_id)) a
							INNER JOIN
					...
					address_verification_data,
					totals_group,
					pan_encrypted FROM	[172.19.75.71].[postilion_office].dbo.post_tran_cust bb )  b 
				ON a.post_tran_cust_id = b.post_tran_cust_id
				...
				print 'Housekeeping: deleting old table post_tran_mega_summary_'+@old_data_timestamp
				exec ('
					if exists (select top 1 * from sys.objects where name = ''post_tran_mega_summary_'+@old_data_timestamp+''' and type =''U'')
					begin
						drop table post_tran_mega_summary_'+@old_data_timestamp+'
					end
				')
				print 'Creating indexes on last table...post_tran_mega_summary_'+@table_date
				exec (
				' 
					CREATE CLUSTERED INDEX [ix_post_tran_'+@date+'_summary_2] ON [dbo].[post_tran_mega_summary_'+@date+'] 
					(
						[post_tran_id] ASC
