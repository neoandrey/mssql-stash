...
ALTER  PROCEDURE manage_transaction_cleaner (@default_retention_period INT, @throttle_factor  INT, @segment_size  INT,@offet_reduction_factor INT) AS
...
BEGIN
	...
	SET @segment_size = ISNULL(@segment_size,8000);
        SET @offet_reduction_factor= ISNULL(@offet_reduction_factor,4);
	...
	SET @new_offet_period = @period_offet /@offet_reduction_factor;
        ...
        END
	SET @new_retention_period = FLOOR(@transaction_period) - FLOOR(@new_offet_period);
