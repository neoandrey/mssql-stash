...
GO
/****** Object:  StoredProcedure [dbo].[usp_copy_settlement_transaction_details]    Script Date: 03/16/2017 13:21:50 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[usp_copy_settlement_transaction_details] (@settlement_date NVARCHAR(10))  AS
							BEGIN
							...
							IF (@start_post_tran_id = 0 OR @start_post_tran_id IS NULL)
							 BEGIN
									...
									IF(OBJECT_ID('tempdb.dbo.##temp_query_table') IS NOT NULL) BEGIN
										...
										DROP TABLE ##temp_query_table;
									END
									IF (@start_post_tran_id=0)BEGIN
										 SELECT   TOP 1 @start_post_tran_id    = (last_post_tran_id) FROM sstl_session (NOLOCK) where CONVERT(VARCHAR(10), datetime_started,112)  =CONVERT(VARCHAR(10), @settlement_date,112) AND completed = 1 order by session_id;
									END
									
									
										EXEC('DECLARE @temp_post_tran_id BIGINT							
												 SELECT    TOP  1 @temp_post_tran_id = (post_tran_id) FROM SSTL_JOURNAL_TEMP (NOLOCK) WHERE CONVERT(VARCHAR(10), business_date,112)  ='+@previous_settlement_date+' ORDER BY  entry_id desc;
												 SELECT   TOP  1 post_tran_id  INTO  ##temp_query_table FROM sstl_journal_temp  (NOLOCK) WHERE post_tran_id >@temp_post_tran_id and business_date ='+@previous_settlement_date+' ORDER BY  entry_id desc')
									IF(OBJECT_ID('tempdb.dbo.##temp_query_table') IS NOT NULL) BEGIN
										SELECT @end_post_tran_id = ISNULL(post_tran_id,0)  FROM  ##temp_query_table;
										DROP TABLE ##temp_query_table;
										END
									ELSE BEGIN
										SELECT   @end_post_tran_id    = MAX(post_tran_id) FROM SSTL_JOURNAL_TEMP WHERE CONVERT(VARCHAR(10), business_date,112)  =@previous_settlement_date;
									END
							...
							END
							;WITH temp_journal_data_local  AS 
							(  SELECT  
							J.adj_id
							,J.entry_id
							,J.config_set_id
							...
							,source_node_key
							,[proc_online_system_id] FROM   post_tran AS PT1 WITH (NOLOCK, INDEX(ix_post_tran_1))
							 where   post_tran_id  BETWEEN '+@start_post_tran_id+'  AND '+@end_post_tran_id+') PT
							...
							;WITH temp_post_tran_data_local AS  (
									SELECT
								PT.[post_tran_id]	PT_post_tran_id
								,PT.[post_tran_cust_id]	PT_post_tran_cust_id
								,PT.[settle_entity_id]	PT_settle_entity_id
							...
							IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''report_result'') AND type in (N''U''))
							DROP TABLE   ##settle_tran_details');
								

															--SELECT   @start_post_tran_id = MIN(post_tran_id)  FROM SSTL_JOURNAL_TEMP   where CONVERT(VARCHAR(10), business_date,112) =@settlement_date;
											...
											SELECT   @end_post_tran_id    = MIN(last_post_tran_id) FROM sstl_session (NOLOCK) where  last_post_tran_id> @start_post_tran_id AND CONVERT(VARCHAR(10), datetime_started,112)  =CONVERT(VARCHAR(10), @settlement_date,112) AND completed = 1;
							--SELECT @start_post_tran_id, @end_post_tran_id, @settlement_date
											
														END
														ELSE
															BEGIN
																SELECT   TOP 1 @end_post_tran_id    = (last_post_tran_id) FROM sstl_session (NOLOCK) where CONVERT(VARCHAR(10), datetime_started,112)  =CONVERT(VARCHAR(10),@settlement_date,112) AND completed = 1 AND last_post_tran_id > @start_post_tran_id order by session_id;
...
,J.coa_config_state 
 FROM 
 (select * from  temp_journal_data_local (NOLOCK) )  J 
                     join 
                 (SELECT * FROM temp_post_tran_data_local (NOLOCK) WHERE PT_tran_postilion_originated =0)   PT 
                    ON (J.post_tran_id = PT.PT_post_tran_id AND J.post_tran_cust_id = PT.PT_post_tran_cust_id and substring(pt.ptc_terminal_id,1,1)!=''G'')
                    left JOIN 
...
,post_tran_cust_id      post_tran_cust_id
,post_tran_id     post_tran_id       				    
FROM  ##settle_tran_details report_results (nolock)
INNER JOIN temp_post_tran_data_local PT 
on PT.PT_post_tran_id = report_results.PTID   
LEFT OUTER JOIN aid_cbn_code acc ON
(acc.acquirer_inst_id1 = PT.PT_acquiring_inst_id_code or acc.acquirer_inst_id2 = PT.PT_acquiring_inst_id_code or 
