...
GO
/****** Object:  StoredProcedure [dbo].[usp_copy_settlement_transaction_details]    Script Date: 04/10/2017 11:24:37 ******/
SET ANSI_NULLS ON
GO
										...
										INCLUDE ([index_no])												  
											...
											DELETE FROM ##settle_tran_details 
											 ...
											 ,''SWTZIBsrc'',''SWTPLATsrc'') and Unique_key IN (select Unique_key from ##settle_tran_details   (NOLOCK) where source_node_name in (''SWTASPPOSsrc'',''SWTASGTVLsrc'')
															...
															');
					EXEC(' SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED	CREATE INDEX ix_settle_tran_details_2  ON  ##settle_tran_details (
									PTID 
							)
