...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner_2]    Script Date: 07/01/2016 18:46:57 ******/
SET ANSI_NULLS ON
GO
...
CREATE     PROCEDURE [dbo].[custom_transaction_cleaner_2] @retention_period INT, @period_interval int , @batch_size int AS  
   ...
   INSERT INTO @recon_business_date_table  SELECT [DATE] FROM dbo.get_dates_in_range(@datetime_start, @last_day_to_clean);  
    ...
    WHILE (DATEDIFF(day,@current_clean_date, @last_day_to_clean)>=0) BEGIN  
      ...
      BEGIN TRY  
        ...
        DELETE_POST_TRAN:  
        DELETE FROM dbo.post_tran   WHERE   post_tran_id in (SELECT  TOP (@batch_Size)  post_tran_id FROM post_tran  WITH (NOLOCK, index(ix_post_tran_7)) WHERE datetime_req >= (SELECT MIN(datetime_req)  FROM post_tran  with( NOLOCK, INDEX(ix_post_tran_7) ) WHERE  datetime_req<=@current_clean_date)  );  
