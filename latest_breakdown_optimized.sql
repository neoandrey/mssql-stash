                        ...
                        left JOIN Reward_Category r (NOLOCK)
                           ...
                           left JOIN tbl_merchant_category_web mer (NOLOCK)
			 						...
			 						OPTION(recompile,optimize for unknown, MAXDOP 8)
														...
														DELETE FROM final_settle_tran_details_tab_20170406
														  WHERE index_no IN (SELECT index_no FROM final_settle_tran_details_tab_20170406 (NOLOCK) where  (source_node_name 
														 IN ('SWTNCS2src','SWTSHOPRTsrc','SWTNCSKIMsrc','SWTNCSKI2src','SWTFBPsrc','SWTUBAsrc'
														 ,'SWTZIBsrc','SWTPLATsrc') and ptid IN (select ptid from final_settle_tran_details_tab_20170406  (NOLOCK) where source_node_name in ('SWTASPPOSsrc','SWTASGTVLsrc')
														)))

			
			 						
			 						
			 						SELECT BANK_CODE,trxn_category,  sum(trxn_amount) amount into temp_details_table	 FROM  settle_tran_details_tab_20170406 (nolock)
