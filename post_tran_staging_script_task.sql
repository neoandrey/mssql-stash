...
(
	...
	INSERT INTO post_tran_summary_staging_info (serverName,tableName,reportDate,reportDate) VALUES (@@SERVERNAME, 'post_tran_summary_'+@tran_date, @tran_date )
...
BEGIN
   Raiseerror('There is no linked server for: '+@serverName+'. Please add a linked server for '+@serverName+' and rerun the job.', 16,1);
   return
END
...
IF (@reportDate IS NULL) BEGIN
	PRINT 'No date specified. Please specify a start date and retry';
	return
END
