...
,(SELECT config_state FROM sstl_se_acc_nr_w CreditAccNr (NOLOCK) WHERE J.credit_acc_nr_id = CreditAccNr.acc_nr_id AND J.config_set_id = CreditAccNr.config_set_id)CreditAccNr_config_state
	...
	,( SELECT description FROM sstl_se_fee_value_w FeeValue (NOLOCK) WHERE J.fee_id = FeeValue.fee_id AND J.config_set_id = FeeValue.config_set_id  AND J.se_id = FeeValue.se_id  AND J.fee_value_id = FeeValue.fee_value_id) FeeValue_description
