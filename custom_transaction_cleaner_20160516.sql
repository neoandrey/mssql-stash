...
GO
/****** Object:  StoredProcedure [dbo].[custom_transaction_cleaner]    Script Date: 05/16/2016 10:24:52 ******/
SET ANSI_NULLS ON
GO
			...
			END
				...
				WHILE (DATEDIFF(day,@current_clean_date, @last_day_to_clean)>=0) BEGIN
						...
						BEGIN TRY
						         ...
						         select @first_post_tran_cust_id = MIN(post_tran_cust_id), @last_post_tran_cust_id = MAX(post_tran_cust_id) FROM post_tran (NOLOCK) WHERE recon_business_date =@current_clean_date OPTION(MAXDOP 12);
								...
								DELETE_POST_TRAN:
								DELETE FROM dbo.post_tran   WHERE   post_tran_cust_id BETWEEN @first_post_tran_cust_id  AND @last_post_tran_cust_id   OPTION(MAXDOP 12);
								...
								DELETE FROM dbo.post_tran_cust   WHERE   post_tran_cust_id BETWEEN @first_post_tran_cust_id  AND @last_post_tran_cust_id   OPTION(MAXDOP 12);
						...
						PRINT 'Cleaning post_tran_cust records older than: '+CONVERT(VARCHAR(30),@current_clean_date)
							...
							PRINT 'Cleaning dbo.post_tran records older than: '+CONVERT(VARCHAR(30),@datetime_stop)
								...
								DELETE_POST_TRAN_5:
								DELETE FROM dbo.post_tran   WHERE   datetime_req <=@current_clean_date  OPTION(MAXDOP 12);
								IF @@ROWCOUNT >0 GOTO DELETE_POST_TRAN_5
								SET ROWCOUNT 0
