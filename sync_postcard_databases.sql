...
begin
		...
		select @logDate =REPLACE(REPLACE(GETDATE(),111),' ', '_'),':','_');
		...
		exec  master.dbo.xp_cmdshell	'for %f in (C:\tabdiff\postcard\*.sql) do sqlcmd /S ASPOFFICE64 /d postcard /E /i "%f" -o postcard_log_'+@logDate;
