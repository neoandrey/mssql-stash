...
create   PROCEDURE [dbo].[psp_settlement_summary_breakdown_optimized](
...
SET @to_date = ISNULL(@end_date,REPLACE(CONVERT(VARCHAR(MAX), GETDATE(),111),'/',''))
	...
	JOIN post_tran PT(NOLOCK, INDEX(ix_post_tran_1))
...
INSERT INTO @temp_late_reversal
  ...
  SELECT t.post_tran_id FROM  post_tran_summary_settle t(NOLOCK)  JOIN tbl_late_reversals ll (NOLOCK) 
		  ON t.post_tran_id = ll.post_tran_id
		   WHERE 
		   ll.recon_business_date >=  REPLACE(CONVERT(VARCHAR(10), '20160507',111),'/', '') 
		   AND  ll.recon_business_date <=  REPLACE(CONVERT(VARCHAR(10), '20160507',111),'/', '')
...
WHEN pt.Retention_data = '1046' and 
...
WHEN pt.Retention_data in ('9030') and 
			...
			WHEN pt.Retention_data = '1131' and 
                         (DebitAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE'
                          OR DebitAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE'
                          OR DebitAccNr.acc_nr LIKE '%AMOUNT_PAYABLE' OR CreditAccNr.acc_nr LIKE '%AMOUNT_PAYABLE') THEN 'WEMA'
                         WHEN pt.Retention_data in ('1061','1006') and 
                         (DebitAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE'
                          OR DebitAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE'

                          OR DebitAccNr.acc_nr LIKE '%AMOUNT_PAYABLE' OR CreditAccNr.acc_nr LIKE '%AMOUNT_PAYABLE') THEN 'GTB'
                         WHEN pt.Retention_data = '1708' and 
                         (DebitAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE'
                          OR DebitAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE'
                          OR DebitAccNr.acc_nr LIKE '%AMOUNT_PAYABLE' OR CreditAccNr.acc_nr LIKE '%AMOUNT_PAYABLE') THEN 'FBN'
                         WHEN pt.Retention_data in ('1027','1045','1081','1015') and 
                         (DebitAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_PAYABLE'
                          OR DebitAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE' OR CreditAccNr.acc_nr LIKE '%ISSUER_FEE_RECEIVABLE'
                          OR DebitAccNr.acc_nr LIKE '%AMOUNT_PAYABLE' OR CreditAccNr.acc_nr LIKE '%AMOUNT_PAYABLE') THEN 'SKYE'
...
END,
  Debit_account_type=CASE 
                     /* WHEN (DebitAccNr.acc_nr LIKE '%AMOUNT%PAYABLE') 
                      ...
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk') THEN 'VISA CO-ACQUIRER AMOUNT PAYABLE(Debit_Nr)'
                      
                      WHEN (DebitAccNr.acc_nr LIKE '%ISSUER%FEE%PAYABLE') 
                      and dbo.fn_rpt_isPurchaseTrx_sett(PT.tran_type, pt.source_node_name, PT.sink_node_name, pt.terminal_id ,pt.totals_group ,pt.pan ) = 1
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk')THEN 'VISA CO-ACQUIRER FEE PAYABLE(Debit_Nr)'*/
                     ...
                     /* WHEN (DebitAccNr.acc_nr LIKE '%ISSUER%FEE%RECEIVABLE') 
                      ...
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk') THEN 'VISA CO-ACQUIRER ISSUER FEE REFUND RECEIVABLE(Debit_Nr)'*/ 
...
END,
  Credit_account_type=CASE  
                      ...
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk') THEN 'VISA CO-ACQUIRER AMOUNT PAYABLE(Credit_Nr)'
                      
                      WHEN (CreditAccNr.acc_nr LIKE '%ISSUER%FEE%PAYABLE') 
                      and dbo.fn_rpt_isPurchaseTrx_sett(PT.tran_type, pt.source_node_name, PT.sink_node_name, pt.terminal_id ,pt.totals_group ,pt.pan ) = 1
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk')THEN 'VISA CO-ACQUIRER FEE PAYABLE(Credit_Nr)'
                      
                      WHEN (CreditAccNr.acc_nr LIKE '%ISSUER%FEE%RECEIVABLE') 
                      and dbo.fn_rpt_isPurchaseTrx_sett(PT.tran_type, pt.source_node_name, PT.sink_node_name, pt.terminal_id ,pt.totals_group ,pt.pan ) = 1
                      and (dbo.fn_rpt_isBillpayment (pt.terminal_id,PT.extended_tran_type,PT.message_type,PT.sink_node_name,PT.payee,pt.card_acceptor_id_code ,pt.source_node_name,pt.tran_type,pt.pan) = 0)
                      AND dbo.fn_rpt_CardGroup(pt.pan) = '6' 
                      AND (pt.source_node_name = 'SWTASPUBAsrc' AND PT.sink_node_name = 'SWTWEBUBAsnk') THEN 'VISA CO-ACQUIRER ISSUER FEE REFUND RECEIVABLE(Credit_Nr)'*/
...
END,
        ...
        Issuer = (case when (not ((DebitAccNr.acc_nr LIKE 'ISW%' and DebitAccNr.acc_nr not LIKE '%POOL%' ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and CreditAccNr.acc_nr not LIKE '%POOL%' ) )) then ''
                      when ((DebitAccNr.acc_nr LIKE 'ISW%' and DebitAccNr.acc_nr not LIKE '%POOL%' ) OR (CreditAccNr.acc_nr LIKE 'ISW%' and CreditAccNr.acc_nr not LIKE '%POOL%' ) ) and (substring(pt.totals_group,1,3) = acc.bank_code1) then acc.bank_code
                        ...
                        --END
	FROM  
	dbo.post_tran_sstl_journal_summary AS J (NOLOCK)
	...
	ON (J.coa_id = Coa.coa_id AND J.config_set_id = Coa.config_set_id)
		RIGHT OUTER JOIN (select * from  post_tran_summary_settle ptt (nolock) where
...
--here
		  --AND not (merchant_type = '5371' and ptt.tran_type = '00' and 

		  --          (dbo.fn_rpt_isPurchaseTrx_sett(ptt.tran_type, ptt.source_node_name, ptt.sink_node_name, ptt.terminal_id ,ptt.totals_group ,ptt.pan) <> 2) 
		  ...
		  and
		  	             ptt.post_tran_id  NOT IN(
		  
		      select post_tran_id from   @temp_late_reversal UNION select post_tran_id from @temp_post_tran_exclusion 
		  ))PT 
	ON (J.post_tran_id = PT.post_tran_id AND J.post_tran_cust_id = PT.post_tran_cust_id and pt.tran_postilion_originated = 1)
	--	left join @temp_post_tran_summary_settle ptt --(nolock) 
	--on (pt.post_tran_cust_id = pt.post_tran_cust_id and pt.tran_postilion_originated = 1
	--	and pt.tran_nr = pt.tran_nr)
