...
THEN  (0.97)* (
 ...
 CASE WHEN RIGHT(dbo.formatTranTypeStr(t.tran_type, t.extended_tran_type, t.message_type) ,2) = '_M' then 0
      WHEN message_type = '0100' AND sink_node_name <> 'VTUsnk' AND rdm_amt = 0  THEN settle_amount_rsp
      WHEN dbo.fn_rpt_isDepositTrx(t.tran_type)  <>1  AND rdm_amt = 0  and sink_node_name <> 'VTUsnk'then -1 * (settle_amount_impact) 
	  WHEN sink_node_name <> 'VTUsnk' AND rdm_amt <> 0  then rdm_amt
  ELSE 0 END
)

 WHEN  LEFT(extended_tran_type,1) IN  ('9', '8') AND Addit_Party in ('ISW','YPM','SAVER') AND tran_type  in ('00','50')
THEN (
...
tran_postilion_originated  = 0
                        ...
                        ON 
						 ...
						 JOIN
						 tbl_merchant_category mch ON  c.merchant_type = mch.category_code
...
c.terminal_id= own.terminal_id
 JOIN tbl_merchant_category me(NOLOCK)
				ON c.merchant_type = me.category_code
