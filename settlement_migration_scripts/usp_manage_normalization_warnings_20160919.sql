...
GO
/****** Object:  StoredProcedure [dbo].[usp_manage_normalization_warnings]    Script Date: 09/19/2016 18:29:33 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
		...
		UPDATE post_tran_exception SET state  = 20 WHERE state =10 AND exception LIKE  '%resubmitted for normalization but this transaction could not be found in the Realtime system%' ;
			...
			UPDATE post_tran_exception SET state  = 40 WHERE  state =10 AND exception LIKE '%a previously logged exception transaction has been detected and will not be normalized%';
