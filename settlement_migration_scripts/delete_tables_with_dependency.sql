...
SELECT * into #sstl_tab_order FROM @tables
 where tableName LIKE '%SSTL%'
 or
 tableName LIKE '%spay%'
 or
 tableName LIKE '%spst%'
...
SELECT * FROM #sstl_tab_order

select ROW_NUMBER() OVER(order by TABLE_NAME) seq_no ,  TABLE_NAME INTO #sstl_config_tables
from INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'config_version' 

SELECT  'DELETE FROM dbo.'+tableName+' WHERE config_version>=@config_version', [level] FROM #sstl_tab_order ord JOIN #sstl_config_tables tabs
ON
ord.tableName = tabs.TABLE_NAME
ORDER BY ord.level
