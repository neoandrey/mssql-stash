...
ALTER TRIGGER  [dbo].[trg_post_node_mapping_stanbic] ON [dbo].[post_online_node]
AFTER INSERT
AS BEGIN
...
WHILE (@@FETCH_STATUS=0)BEGIN
	...
	FETCH NEXT FROM switch_column_data_cursor INTO @super_switch_online_node_name
END
CLOSE switch_column_data_cursor
DEALLOCATE switch_column_data_cursor
IF ( LEN(@merged_switch_online_node_data)>1 ) BEGIN
...
set @merged_switch_online_node_data ='';
DECLARE mega_column_data_cursor CURSOR LOCAL FORWARD_ONLY STATIC READ_ONLY FOR SELECT DISTINCT online_node_name FROM post_online_node (NOLOCK) WHERE  RIGHT(online_node_name, 3)='snk' AND  online_node_name NOT LIKE '%CHB%' AND online_system_id IN (@mega_switch_live_system,@mega_switch_dr_system)
OPEN mega_column_data_cursor
FETCH NEXT FROM mega_column_data_cursor INTO @mega_switch_online_node_name
WHILE (@@FETCH_STATUS=0)BEGIN
	SET @merged_switch_online_node_data =@merged_switch_online_node_data+@mega_switch_online_node_name+',';
	FETCH NEXT FROM mega_column_data_cursor INTO @mega_switch_online_node_name
END
CLOSE mega_column_data_cursor
DEALLOCATE mega_column_data_cursor
IF ( LEN(@merged_switch_online_node_data)>1 ) BEGIN
	SET @merged_switch_online_node_data = SUBSTRING(@merged_switch_online_node_data, 1, LEN(@merged_switch_online_node_data)-1)
...
set @merged_switch_online_node_data ='';
DECLARE asp_column_data_cursor CURSOR LOCAL FORWARD_ONLY STATIC READ_ONLY FOR SELECT DISTINCT online_node_name FROM post_online_node (NOLOCK) WHERE RIGHT(online_node_name, 3)='snk' AND  online_node_name NOT LIKE '%CHB%' AND online_system_id IN(@asp_switch_live_system,@asp_switch_dr_system)
OPEN asp_column_data_cursor
FETCH NEXT FROM asp_column_data_cursor INTO @asp_switch_online_node_name
WHILE (@@FETCH_STATUS=0)BEGIN
	SET @merged_switch_online_node_data =@merged_switch_online_node_data+@asp_switch_online_node_name+',';
	FETCH NEXT FROM asp_column_data_cursor INTO @asp_switch_online_node_name
END
CLOSE asp_column_data_cursor
DEALLOCATE asp_column_data_cursor
IF ( LEN(@merged_switch_online_node_data)>1 ) BEGIN
	SET @merged_switch_online_node_data = SUBSTRING(@merged_switch_online_node_data, 1, LEN(@merged_switch_online_node_data)-1)
